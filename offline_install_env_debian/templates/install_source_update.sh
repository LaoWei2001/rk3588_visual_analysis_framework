...
echo "     主程序编译和安装："
echo "       cd $SOURCE_INSTALL_PATH/vision_analysis"
echo "       ./build.sh dist"
echo "       sudo ./install_app.sh dist"
echo "     首次网络配置工具编译："
echo "       cd $SOURCE_INSTALL_PATH/first_net_config && ./build.sh"
echo "     Web 控制台源码有修改时，重新构建并部署："
echo "       cd $SOURCE_INSTALL_PATH/web_console/frontend && npm run build"
echo "       cd $SOURCE_INSTALL_PATH && sudo ./install.sh upgrade offline"
