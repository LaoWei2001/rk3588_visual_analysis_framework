...
# 用法（在项目根目录执行）：
#
#   bash offline_install_env_debian/create_source_update.sh
#
...
# 输出目录：
#
#   offline_install_env_debian/output/source-update
#
...
# 查看帮助：
#
#   bash offline_install_env_debian/create_source_update.sh --help
set -Eeuo pipefail
...
usage() {
    cat <<'EOF'
用法：bash offline_install_env_debian/create_source_update.sh
...
if ! cmp -s "$STAGE_DIR/base-dependencies.sha256" "$STAGE_DIR/current-dependencies.sha256"; then
    ...
    echo "       请运行: bash offline_install_env_debian/create_bundle.sh --full" >&2
    exit 1
fi
...
首次网络配置工具编译：
  cd $SOURCE_INSTALL_PATH/first_net_config
  ./build.sh
EOF
