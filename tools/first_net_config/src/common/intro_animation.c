...
enum { INTRO_MATERIAL_MASK = 1, INTRO_MATERIAL_VOID };

static void intro_plot(IntroFrame *frame, double x, double y, double z,
  ...
  normal_length = sqrt(rotated_nx * rotated_nx + rotated_ny * rotated_ny +
                       rotated_nz * rotated_nz);
  if (normal_length < 0.0001) {
    normal_length = 1.0;
  }
  light = (-0.35 * rotated_nx + 0.55 * rotated_ny + 0.95 * rotated_nz) /
          normal_length;
  light = (light + 1.0) * 0.5;
  if (light < 0.08) {
    light = 0.08;
...
static double intro_mask_half_width(double y) {
  static const double profile[][2] = {
      {-1.48, 0.12}, {-1.08, 0.26}, {-0.58, 0.42}, {0.02, 0.59},
      {0.53, 0.72},  {1.00, 0.68},  {1.33, 0.45}};
...
static bool intro_inside_mask_eye(double x, double y) {
  ...
  double ellipse = (u * u) / (0.22 * 0.22) + (v * v) / (0.29 * 0.29);

  /* The lower taper gives the iconic long, mournful eye openings. */
  return ellipse < 1.0 && fabs(u) < 0.19 - 0.035 * (0.29 - v);
}
...
static bool intro_inside_mask_mouth(double x, double y) {
  ...
  return pow(fabs(horizontal), 1.7) + pow(fabs(vertical), 2.2) < 1.0;
}
...
static void intro_build_ghostface_mask(IntroFrame *frame) {
  ...
  for (int vertical = 0; vertical <= 82; ++vertical) {
    ...
    for (int horizontal = -38; horizontal <= 38; ++horizontal) {
      ...
      double nose_relief =
          0.16 * exp(-12.0 * x * x) * exp(-3.0 * (y - 0.12) * (y - 0.12));
      double cheek_relief = 0.045 * cos(y * 4.2) * (1.0 - position * position);
      double z = 0.70 + 0.28 * (1.0 - position * position) + nose_relief +
                 cheek_relief;

      if (intro_inside_mask_eye(x, y) || intro_inside_mask_nose(x, y) ||
          intro_inside_mask_mouth(x, y)) {
        intro_plot(frame, x, y, z + 0.012, 0.0, 0.0, 1.0, INTRO_MATERIAL_VOID);
        continue;
      }
      intro_plot(frame, x, y, z, 0.78 * position, -0.08 * sin(y * 4.2), 1.0,
                 INTRO_MATERIAL_MASK);
    }
  }
  ...
  for (int vertical = 0; vertical <= 82; ++vertical) {
    ...
    for (int side = -1; side <= 1; side += 2) {
      for (int layer = 0; layer <= 8; ++layer) {
        ...
        intro_plot(frame, side * half_width, y, z, side, 0.0, 0.25,
                   INTRO_MATERIAL_MASK);
      }
    }
  }
...
void intro_animation_play(void) {
  ...
  for (int animation_frame = 0; animation_frame < INTRO_FRAMES;
       ++animation_frame) {
    int key;
    ...
    frame.angle = 2.0 * INTRO_PI * animation_frame / (INTRO_FRAMES - 1);
