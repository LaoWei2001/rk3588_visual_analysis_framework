...
int choose_or_enter_camera(const char *iface, DiscoveredCamera *camera)
{
    ...
    terminal_ui_set_step("扫描摄像头",
                         "等待局域网内的 SADP / ONVIF 设备响应");
    terminal_ui_show_busy("正在扫描摄像头...",
                          "正在发送发现报文并收集响应，预计等待 4 秒。");
    printf("本次仅将 %s 用于摄像头；程序不固定网口角色。\n", iface);
...
camera_selection_step:
    if (count > 0)
    {
        terminal_ui_set_step("选择摄像头",
                             "扫描已完成；选择设备或手动填写地址");
        printf("\n发现摄像头：\n");
        ...
        printf("  0. 返回上一步\n");
        {
            int choice = read_int("请选择摄像头: ", 0, count + 1);
    ...
    {
        printf("未发现摄像头；部分固件/交换机不转发发现报文，可手动填写。\n");
    }
    ...
    camera->http_port = 80;
    camera->rtsp_port = 554;
    terminal_ui_set_step("手动填写摄像头",
                         "请输入摄像头当前的 IPv4 地址");
    terminal_ui_prepare_step(true);
    if (!read_ipv4_required("摄像头 IPv4 地址: ",
                            camera->ip, sizeof(camera->ip)))
    {
        if (terminal_ui_back_requested() && count > 0)
        {
            goto camera_selection_step;
        }
        return 0;
    }
    return 1;
