...
def measure(binary, config, seconds, output, label, trial, cwd, warmup_seconds=5):
    ...
    if not values or len(steady_samples) < 2 or proc.returncode != 0:
        raise RuntimeError(f'No valid completed measurement: {log}; exit={proc.returncode}')
    return {
...
def main():
    ...
    if args.seconds < 20:
        parser.error('Use at least 20 seconds to collect steady-state samples')
    if args.warmup_seconds < 5 or args.warmup_seconds % 5 or args.seconds < args.warmup_seconds + 15:
        parser.error('Warmup must be a positive multiple of five, leaving at least 15 seconds of measurements')
