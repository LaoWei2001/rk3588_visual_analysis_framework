...
def test_external_project_lifecycle_and_version_rejection(tmp_path):
    ...
    subprocess.run([str(ROOT / 'vision'), 'new-logic', str(project), 'global', 'global_test'], check=True)
...
def test_local_project_launcher_uses_selected_engine(tmp_path):
    project = tmp_path / 'local-app'
    vision.create(project)
    subprocess.run(['bash', str(project / 'build.sh'), '--help'], cwd=tmp_path, check=True)
