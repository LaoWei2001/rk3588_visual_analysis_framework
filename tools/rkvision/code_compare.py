...
def code_sections(path):
    ...
    if data[:6] != b'\x7fELF\x02\x01':
        raise ValueError(f'文件不是小端 ELF64 格式：{path}')
...
def compare(before,after):
    ...
    if not old:
        raise ValueError(f'基准构建目录中没有找到目标文件：{before}')
...
def main(argv=None):
    ...
    return int(bool(result["different_executable_sections"] or result["missing_or_ambiguous"]))
