#!/usr/bin/env python3
"""Create a channel or global Logic module with the shared manifest contract."""
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    if not IDENTIFIER_RE.fullmatch(args.name):
        raise ValueError(f"Logic 名称无效：{args.name}（必须是合法的 C++ 标识符）")
    ...
    if module_dir.exists():
        raise ValueError(f"Logic 模块已存在：{module_dir}")
