...
本目录是 `rkvision create` 使用的最小项目模板。逻辑、资源、配置和构建产物均属于本项目。

```bash
/path/to/framework/rkvision check .
/path/to/framework/rkvision build .
/path/to/framework/rkvision build . --profile debug
/path/to/framework/rkvision package .
/path/to/framework/rkvision clean .
```
...
引擎选择：使用指定框架目录下的 `rkvision`，或在 CMake 中设置 `-DRKVISION_ENGINE`。
...
`rkvision build` 先增量构建共用引擎缓存，再编译本项目；直接 CMake 默认在本项目构建目录生成共享库。
