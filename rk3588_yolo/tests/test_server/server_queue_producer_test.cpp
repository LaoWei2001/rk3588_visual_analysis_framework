...
static bool load_config(const std::string &path, Args &args)
{
    ...
    while (std::getline(fin, line))
    {
        ...
        {
            if (key_raw == "host")
                args.redis_host = val_raw;
            else if (key_raw == "port")
            {
                int v = 0;
                if (parse_positive_int(val_raw, v))
                    args.redis_port = v;
            }
        }
        ...
        else if (section == "test")
        {
            ...
            if (key_raw == "camera_id")
            {
                int v = 0;
                if (std::stoi(val_raw, nullptr) >= 0)
                    args.camera_id = std::stoi(val_raw);
            }
            if (key_raw == "alarm_type" && !val_raw.empty())
                args.alarm_type = val_raw;
        }
    }
...
static bool parse_args(int argc, char **argv, Args &args)
{
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (key == "--image-draw")
        {
            const char *v = need_value("--image-draw");
            if (!v)
                return false;
            args.image_draw_path = v;
        }
        else if (key == "--image-raw")
        {
            const char *v = need_value("--image-raw");
            if (!v)
                return false;
            args.image_raw_path = v;
        }
        else if (key == "--config")
        {
