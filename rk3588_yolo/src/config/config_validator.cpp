...
bool ConfigValidator::is_valid_url(const std::string& url) {
    ...
    if (url.substr(0, 7)  == "rtsp://")  return true;
    if (url.substr(0, 8)  == "rtsps://") return true;
    if (url.substr(0, 7)  == "http://")  return true;
    if (url.substr(0, 8)  == "https://") return true;
    if (url[0] == '/')                    return true; // 本地文件绝对路径
    return false;
}
...
bool ConfigValidator::validate_channels(const AppConfig& cfg, std::vector<ValidationError>& errors) {
    ...
    if (cfg.enable_display && grid_capacity < enabled_count) {
        char buf[256];
        snprintf(buf, sizeof(buf), "显示网格容量不足: 需要%d个单元格，但只有%d个 (%d*%d)",
                 enabled_count, grid_capacity, cfg.tile_cols, cfg.tile_rows);
