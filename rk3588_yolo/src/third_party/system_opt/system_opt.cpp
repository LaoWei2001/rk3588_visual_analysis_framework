...
************************************************************/
#define TRY_EVALUATE_POINTER(pointer, value)                                                                           \
    do                                                                                                                 \
    {                                                                                                                  \
        if (pointer)                                                                                                   \
        {                                                                                                              \
...
Example:
        double cpu_usage()
    {
        ...
        //计算cpu使用率
        cpu = cal_cpu_occupy ((cpu_occupy_t *)&cpu_stat1, (cpu_occupy_t
*)&cpu_stat2); memcpy(&cpu_stat1, &cpu_stat2, sizeof(cpu_occupy_t));
...
double cal_cpu_occupy(cpu_occupy_t *o, cpu_occupy_t *n)
{
    ...
    if ((nd - od) != 0)
        cpu_use =
            100.0 - ((id - sd)) / (nd - od) * 100.00; //((用户+系统)乘100)除(第一次和第二次的时间差)再赋给g_cpu_used
    else
        cpu_use = 0;
...
Return:
        无
说明:
        - nanosleep一旦被调用，进程就进入 TASK_INTERRUPTIBLE
状态，直到进程被唤醒，就回到 TASK_RUNNIN 状态。
        - TASK_INTERRUPTIBLE 和 TASK_UNINTERRUPTIBLE 的区别：
                TASK_INTERRUPTIBLE 是可以被 [信号] 和 [wake_up()]
唤醒的，当信号到来时，进程会被设置为可运行。 TASK_UNINTERRUPTIBLE 只能被
[wake_up()] 唤醒。
********************************************************************/
...
Return:
        ...
        (注意，command顺利执行不代表执行成功，比如command："rm
debuglog.txt"，不管文件存不存在该command都顺利执行了)
...
注意：
        ...
        * 如果你希望使用 wait () 或 waitpid ()
对子进程收尸，那么你必须在调用前(事实上是fork
()前)将SIGCHLD信号置为SIG_DFL处理方式。
...
static int32_t SYSTEM(const char *cmdstring)
{
    ...
    //         -保证子进程先运行，在调用 exec 或 exit
    //         之前与父进程数据是共享的。父进程在子进程调用 exec 或 exit
    //         之后才可能被调度运行，如果在调用这两个函数之前子进程依赖于父进程的进一步动作，则会导致死锁。
    if ((pid = vfork()) < 0)
    {
        status = -1;
    }
...
注意：
        如果 SIGCHLD 信号行为被设置为 SIG_IGN 时，waitpid ()
函数有可能因为找不到子进程而报 ECHILD 错误。 解析：
    * systeme() 函数依赖了系统的一个特性，那就是内核初始化进程时对 SIGCHLD
信号的处理方式为 SIG_DFL。

        -[信号的处理方式为 SIG_DFL]是什么什么意思呢？
        * 即内核发现进程的子进程终止后给进程发送一个 SIGCHLD
信号，进程收到该信号后采用 SIG_DFL 方式处理。

        -那么 SIG_DFL 又是什么方式呢？
        * SIG_DFL
是一个宏，定义了一个信号处理函数指针，事实上该信号处理函数什么也没做。这个特性正是
system () 函数需要的。
        * system () 函数首先 fork () 一个子进程执行 command 命令，执行完后
system () 函数会使用 waitpid () 函数对子进程进行收尸。
********************************************************************/
...
注意：
        - 如果 cmd
执行失败，子进程会把错误信息打印到标准错误输出，父进程就无法获取。
...
int32_t exec_cmd_by_popen(const char *cmd, char *result)
{
    ...
    if ((ptr = popen(ps, "r")) != NULL)
    {
        ...
        {
            size_t buf_len = strlen(buf_ps);
            if (result_len + buf_len < 1024)
            {
                memcpy(result + result_len, buf_ps, buf_len);
                result_len += buf_len;
                result[result_len] = '\0';
            }
            else
            {
                /* 缓冲区即将溢出, 截断并退出 */
                size_t remain = 1024 - result_len - 1;
                if (remain > 0)
                {
                    memcpy(result + result_len, buf_ps, remain);
                    result_len += remain;
                    result[result_len] = '\0';
                }
                break;
