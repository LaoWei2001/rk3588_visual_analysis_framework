...
class ModelBase
{
  public:
    ...
    /**
     * @brief 返回 NPU 输入 buffer 的预缓存 RGA handle (rga_buffer_handle_t 即
     * int). 模型初始化时一次性 importbuffer_fd, 析构时 releasebuffer_handle.
    ...
    /**
     * @brief Whether the model already performs NMS internally during
     * post_process. If true, the pipeline will skip the redundant nms_inplace()
     * call.
     */
