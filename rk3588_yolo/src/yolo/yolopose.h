...
public:
    YoloPose(const std::string& model_path, const std::string& label_path,
...
private:
    ...
    float keypoint_value(const float* buffer, int keypoint_index,
...
private:
     ...
     * 关键点输出支持 [K*3, N] / [K,3,N] 及 [N,K*3] / [N,K,3]。 */
