...
CompositeModel::CompositeModel(std::vector<Entry> entries)
...
bool CompositeModel::infer(cv::Mat &frame, std::vector<AlgoResult> &results, YoloPerfStat *perf)
{
    ...
    for (size_t model_index = 0; model_index < entries_.size(); ++model_index)
    {
        ...
        const bool ok = entry.model->infer(frame, child_results[model_index],
                                           &child_perf[model_index]);
...
bool CompositeModel::merge_child_results(
    std::vector<std::vector<AlgoResult>> &child_results,
    ...
    YoloPerfStat *perf) const
{
    ...
    const size_t count = std::min(entries_.size(),
        std::min(child_results.size(), std::min(succeeded.size(), child_perf.size())));
    for (size_t model_index = 0; model_index < count; ++model_index)
    {
        ...
        one_model_results.erase(
            std::remove_if(one_model_results.begin(), one_model_results.end(),
