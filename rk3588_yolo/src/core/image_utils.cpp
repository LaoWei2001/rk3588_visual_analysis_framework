...
bool raw_to_bgr_mat(const void *pSrcData, int srcW, int srcH, int srcStrH, int srcStrV, int srcFmt, cv::Mat &out)
{
    ...
    {
        log_printf_threadsafe("[ImageUtils] Invalid input arguments! pSrcData=%p, "
                              "w=%d, h=%d, strW=%d, strH=%d\n",
                              pSrcData, srcW, srcH, srcStrH, srcStrV);
        return false;
    }

    // NV12 = 0x1, NV21 = 0x2, BGR = 0x3, RGB = 0x4 (RK_FORMAT_*)
    // Wait, the caller passes either RK_FORMAT_* or pixel_format from
    // channel_raw. Let's rely on RK_FORMAT_ constants from RgaApi.h
    // RK_FORMAT_YCbCr_420_SP = 0x0A, RK_FORMAT_YCrCb_420_SP = 0x0B
    // RK_FORMAT_BGR_888 = 0x0D, RK_FORMAT_RGB_888 = 0x0E

    // Extract base format using mask, as RK_FORMAT_ constants might have
    // context/version high bits
