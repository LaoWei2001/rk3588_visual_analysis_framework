...
cv::Ptr<cv::freetype::FreeType2> ensure_ft2_locked()
{
    ...
    if (!g_ft2)
        fprintf(stderr,
                "[text_overlay][ERROR] 未能加载任何字体! 画面文字将无法显示。\n"
...
{
    if (utf8.empty() || img.empty()) return false;
    std::lock_guard<std::mutex> lk(g_ft_mtx);
    cv::Ptr<cv::freetype::FreeType2> ft = ensure_ft2_locked();
    if (!ft) return false;
    if (font_height_px < 1) font_height_px = 1;
    try {
        ft->putText(img, utf8, org, font_height_px, color, thickness,
                    cv::LINE_AA, /*bottomLeftOrigin=*/true);
    } catch (...) {
        return false;
    }
    return true;
