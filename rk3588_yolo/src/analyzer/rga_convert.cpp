...
bool rga_convert_resize(int chnId, const RgaImage &src_img, const RgaImage &dst_img)
{
    rga_buffer_t src = wrapbuffer_virtualaddr(
        src_img.pBuf, src_img.width, src_img.height,
        src_img.fmt, src_img.hor_stride, src_img.ver_stride);
    src.fd = -1;

    rga_buffer_t dst = wrapbuffer_virtualaddr(
        dst_img.pBuf, dst_img.width, dst_img.height,
        dst_img.fmt, dst_img.hor_stride, dst_img.ver_stride);
    dst.fd = -1;
...
std::shared_ptr<RgaImportedBuffer> rga_import_src_fd(
    int fd, int w, int h, int stride_w, int stride_h, int fmt)
{
    if (fd < 0 || w <= 0 || h <= 0 || stride_w <= 0 || stride_h <= 0) return nullptr;
    ...
    if (h_id == 0)
    {
        ...
        if (c <= 20 || (c % 200) == 0)
            fprintf(stderr,
                    "[RGA] importbuffer_fd failed cnt=%d  fd=%d %dx%d stride=%dx%d fmt=%d\n",
...
bool rga_convert_resize_handle(int chnId, const RgaImportedBuffer &src,
                                ...
                                int dst_stride_w, int dst_stride_h, int dst_fmt,
                                int cached_dst_handle)
{
    if (src.handle == 0 || dst_fd < 0) return false;
    ...
    else
    {
        ...
        if (dst_handle == 0)
        {
            ...
            if (c <= 20 || (c % 200) == 0)
                fprintf(stderr,
                        "[RGA] importbuffer_fd dst failed cnt=%d  fd=%d %dx%d stride=%dx%d\n",
    ...
    rga_buffer_t src_buf = wrapbuffer_handle(
        src.handle, src.width, src.height, src.format, src.stride_w, src.stride_h);
    rga_buffer_t dst_buf = wrapbuffer_handle(
        dst_handle, dst_w, dst_h, dst_fmt, dst_stride_w, dst_stride_h);
    ...
    if (!handle_is_cached) releasebuffer_handle(dst_handle);
...
bool convertToYoloInput(int chnId, void *pSrcData, int src_fd,
                         int srcW, int srcH, int srcStrH, int srcStrV,
                         int srcFmt, cv::Mat &out)
{
    if (srcW <= 0 || srcH <= 0 || srcStrH <= 0 || srcStrV <= 0 ||
        (!pSrcData && src_fd < 0))
    {
        static std::atomic<int> cnt{0};
        int c = ++cnt;
        if (c <= 20 || (c % 200) == 0)
            fprintf(stderr,
                    "[yolo_in] skip invalid frame cnt=%d  src=%dx%d stride=%dx%d fd=%d\n",
                    c, srcW, srcH, srcStrH, srcStrV, src_fd);
        return false;
    }
        ...
        srcFmt == RK_FORMAT_BGR_888       || srcFmt == RK_FORMAT_RGB_888)
    {
        ...
        if (src_fd >= 0)
        {
            ...
            rga_buffer_t pat{};
            im_rect prect{};
            rga_ok = (improcess(src, dst, pat, srect, drect, prect,
                                0, nullptr, &opt, 0) == IM_STATUS_SUCCESS);
        }
        else
        {
            RgaImage src_img;
        ...
        if (rga_ok) return true;

        static std::atomic<int> cnt{0};
        int c = ++cnt;
        if (c <= 20 || (c % 200) == 0)
            fprintf(stderr,
                    "[yolo_in] RGA fail cnt=%d  src=%dx%d stride=%dx%d fmt=%d\n",
                    c, srcW, srcH, srcStrH, srcStrV, srcFmt);
    }
...
{
    if (0 == strcmp(strFmt, "NV12")) return RK_FORMAT_YCbCr_420_SP;
    if (0 == strcmp(strFmt, "NV21")) return RK_FORMAT_YCrCb_420_SP;
    if (0 == strcmp(strFmt, "BGR"))  return RK_FORMAT_BGR_888;
    if (0 == strcmp(strFmt, "RGB"))  return RK_FORMAT_RGB_888;
