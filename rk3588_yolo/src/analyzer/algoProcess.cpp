...
bool algorithm_take_results(int chnId, std::vector<AlgoResult> &out,
                             cv::Mat &out_frame, int64_t &out_frame_id)
{
    out.clear(); out_frame.release(); out_frame_id = 0;
    if (chnId < 0 || chnId >= MAX_CHANNEL_NUM) return false;
...
int algorithm_init(const AppConfig &cfg)
{
    ...
    for (int i = 0; i < MAX_CHANNEL_NUM; ++i) g_algo.result_dispatch_pending[i] = 0;

    for (size_t i = 0; i < cfg.channels.size() && i < MAX_CHANNEL_NUM; ++i)
    {
        const auto &chn_cfg   = cfg.channels[i];
        int         runtime_chn = (int)i;
        if (!chn_cfg.enable) continue;
        ...
        try
        {
            ...
            std::string label_path = chn_cfg.label_path.empty()
                                     ? cfg.label_path : chn_cfg.label_path;

            for (int t = 0; t < threads_for_chn; ++t)
            {
                int mask, core_id;
                if (chn_cfg.npu_core >= 0 && chn_cfg.npu_core <= 2)
                { core_id = chn_cfg.npu_core; mask = core_masks[core_id]; }
                else
                { core_id = auto_model_instances % 3; mask = core_masks[core_id]; auto_model_instances++; }
                ...
                if (t == 0)
                {
                    ...
                    if (it != g_algo.model_registry.end())
                    {
                        ...
                        log_printf_threadsafe("[Algo] Reusing %s instance for ch%d (Core %d) [shared]\n",
                                              type.c_str(), runtime_chn, core_id);
                    }
                    else
                    {
                        model = create_model(type, chn_cfg.model_path, label_path,
                                             mask, chn_cfg.obj_thresh, chn_cfg.nms_thresh);
                        if (!model)
                        { printf("[Algo] Unsupported model_type '%s' for channel %d\n",
                                 type.c_str(), runtime_chn); continue; }
                ...
                else
                {
                    ...
                    { printf("[Algo] Unsupported model_type '%s' for channel %d\n",
                             type.c_str(), runtime_chn); continue; }
                    log_printf_threadsafe("[Algo] Created %s instance for ch%d (Core %d, thread %d/%d) [pipeline]\n",
                                          type.c_str(), runtime_chn, core_id, t, threads_for_chn);
                }

                if (loaded_models_count == 0)
                { g_algo.input_w = model->input_width(); g_algo.input_h = model->input_height(); }
    ...
    if (!g_algo.running)
    {
        ...
        for (int i = 0; i < MAX_CHANNEL_NUM; ++i)
        { g_fps[i].init(now); g_perf[i].last_log_ms = now_ms; }
...
{
    g_algo.running = 0;
    for (auto &tq : g_algo.task_queues) { pthread_cond_broadcast(&tq->cv); }
    for (int i = 0; i < MAX_CHANNEL_NUM; ++i) pthread_cond_broadcast(&g_algo.result_ready_cv[i]);

    for (pthread_t tid : g_algo.worker_tids) pthread_join(tid, nullptr);
    ...
    g_algo.task_queues.clear();
    for (auto &models : g_algo.models_per_chn) models.clear();
    g_algo.model_registry.clear();
...
int algorithm_process_mat(int chnId, cv::Mat &&frame, int fd,
                           int srcW, int srcH, int srcFmt, int srcStrH, int srcStrV,
                           int64_t frame_seq)
{
    if (!g_algo.running) return -1;
    if (frame.empty() && fd < 0) return -1;

    pthread_rwlock_rdlock(&g_algo.dispatch_mtx);
    if (!g_algo.running) { pthread_rwlock_unlock(&g_algo.dispatch_mtx); return -1; }
    if (g_algo.task_queues.empty()) { pthread_rwlock_unlock(&g_algo.dispatch_mtx); return -1; }
    if (chnId >= 0 && chnId < MAX_CHANNEL_NUM && g_algo.chn_reload_stop[chnId])
    { pthread_rwlock_unlock(&g_algo.dispatch_mtx); return -1; }
    ...
    for (int i = 0; i < MAX_CHANNEL_NUM; ++i)
    {
        if (!g_algo.models_per_chn[i].empty())
        {
            if (i == chnId)
            {
                ...
                { pthread_mutex_unlock(&tq.mtx); pthread_rwlock_unlock(&g_algo.dispatch_mtx); return 0; }
                ...
                if (fd >= 0) task.src_buf = rga_import_src_fd(fd, srcW, srcH, srcStrH, srcStrV, srcFmt);
                task.srcW = srcW; task.srcH = srcH; task.srcFmt = srcFmt;
                task.srcStrH = srcStrH; task.srcStrV = srcStrV;
...
int algorithm_get_input_w() { return g_algo.input_w; }
int algorithm_get_input_h() { return g_algo.input_h; }

float algorithm_get_infer_fps(int chnId)
{
    if (chnId < 0 || chnId >= MAX_CHANNEL_NUM) return 0.0f;
...
bool algorithm_wait_result(int chnId, int timeout_ms)
{
    ...
    if (!g_algo.result_dispatch_pending[chnId] && g_algo.running)
    {
        ...
        pthread_cond_timedwait(&g_algo.result_ready_cv[chnId],
                               &g_algo.result_ready_mtx[chnId], &ts);
    }
...
void algorithm_update_detect_classes(int chnId, const std::vector<std::string> &class_names)
{
    ...
    if (g_pCtrl) label_path = g_pCtrl->config.channels[chnId].label_path;
    auto ids = std::make_shared<const std::set<int>>(
        names_to_class_ids(class_names, label_path));
...
{
    if (chnId < 0 || chnId >= MAX_CHANNEL_NUM || !g_algo.running) return;
    ...
    log_printf_threadsafe("[Algo] Reloading model for ch%d: type=%s path=%s\n",
                          chnId, new_cfg.model_type.c_str(), new_cfg.model_path.c_str());
    ...
    auto restart_workers = [&](const std::vector<std::shared_ptr<ModelBase>> &new_models)
    {
        ...
        for (int w = worker_start; w < worker_start + worker_count; ++w)
        {
            ...
            WorkerArg *wa = new WorkerArg{
                chnId, tq,
                m_idx < (int)new_models.size() ? new_models[m_idx] : std::shared_ptr<ModelBase>()
            };
    ...
    if (label_path.empty() && g_pCtrl) label_path = g_pCtrl->config.label_path;
    ...
    try
    {
        for (int t = 0; t < worker_count; ++t)
        {
            ...
            auto model = create_model(new_cfg.model_type, new_cfg.model_path,
                                       label_path, mask,
                                       new_cfg.obj_thresh, new_cfg.nms_thresh);
            if (!model)
            { log_printf_threadsafe("[Algo] Unsupported model_type '%s' for ch%d\n",
                                    new_cfg.model_type.c_str(), chnId); continue; }
