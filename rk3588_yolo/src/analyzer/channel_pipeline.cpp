...
void analyzer_update_tracker(int chnId, const ChannelConfig *ch)
{
    ...
    if (ch->tracker_enable)
    {
        ...
        {
            g_trackers[chnId] =
                std::make_unique<Tracker>(ch->tracker_iou_thresh, ch->tracker_max_miss, ch->tracker_min_hits);
            printf("[ChannelPipeline] tracker enabled for ch%d (iou=%.2f, miss=%d)\n", chnId, ch->tracker_iou_thresh,
                   ch->tracker_max_miss);
        }
    }
    else
 ...
 *   fn(&ctx) 在锁外运行；仅写回 last_logic_frame/last_results/draw_cmds
 * 时短暂持锁。 这使 get_channel_snapshot() 等待时间从"logic
 * 执行时长"降至"赋值时长"（μs 级）。
 *
 * @param chnId          通道号
                                 ...
                                 int64_t frame_id, uint64_t timestamp_ms, float dt_ms, int infer_enabled)
{
    ...
    /* 墙上时钟(epoch ms): RTSP/USB/文件 三源统一在此盖一次, logic 读 ctx->unix_ms
     * / time_hms() 即得本帧真实时间。这是"处理本帧的时刻",
     * 与采集相差一个管线延迟(对 HH:MM:SS 显示无感)。 */
    ...
    /* 多 ROI: ctx.rois = 本通道全部区域; ctx.roi = 第一个区域(兼容老逻辑,
     * 无区域时为 nullptr)。
     * 顶点已是模型坐标系(归一化加载时即是; 旧像素格式由下方
     * process_channel_results 缩放后填入)。*/
    ...
    /* logic 在锁外运行：frame_for_logic/current_results/draw_cmds
     * 均为调用栈本地变量； ctx.roi 和 ctx.state 受外层 g_process_mtx
     * 保护，ctx.config 与热重载共存 （config 指针在 logic
     * 执行期间不会失效，只存在极低概率的值新旧混读，与改前相同）。 chn_mtx
     * 只用于最终写回，持锁时间从 ms 级降至 μs 级。*/
                                                ...
                                                int64_t result_frame_id)
{
    ...
    {
        pthread_rwlock_rdlock(&g_pCtrl->mtx);
        infer_enabled = config_utils::is_channel_infer_enabled(g_pCtrl->config.channels[chnId]) ? 1 : 0;
        pthread_rwlock_unlock(&g_pCtrl->mtx);
    }

    /* ROI 区域 → 逻辑坐标系(模型 640)。逻辑与显示都只用
     * roi_zones[i].polygon(模型坐标)，与视频源无关。 归一化坐标已由
     * load_roi_zones_from_config 在初始化时 × 模型尺寸转换为模型坐标系。*/
    ...
    if (!infer_enabled || !new_results)
    {
        ...
        const float dt_ms = (ch_state.logic_frame_id <= 1) ? 0.0f : (float)(now_ms - ch_state.last_logic_ts_ms);
        ...
        if (!raw_frame.model_input_mat.empty())
        {
            invoke_channel_logic(chnId, raw_frame.model_input_mat, empty_results, ch_state.logic_frame_id, now_ms,
                                 dt_ms, infer_enabled);
        }
    ...
    if (frame_for_logic.empty())
        return out;
    invoke_channel_logic(chnId, frame_for_logic, out, frame_seq, frame_ts, dt_ms, infer_enabled);
    return out;
}
