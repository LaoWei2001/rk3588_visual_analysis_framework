 ...
 *   ③ display_lock / display_unlock 保护 copyTo 到 front_roi 的操作（避免 GTK
 * 撕裂）。 ④ RGA 段（rga_convert_resize 调用）不在 display_lock
 * 内，避免把锁争用误报为 RGA 错误。
 */
                           ...
                           int srcVStride)
{
    ...
    if (!pSrcData || srcWidth <= 0 || srcHeight <= 0 || srcHStride <= 0 || srcVStride <= 0)
    {
        ...
        if (c <= 20 || (c % 200) == 0)
            fprintf(stderr,
                    "[commit] skip invalid frame ch=%d cnt=%d  src=%dx%d "
                    "stride=%dx%d pBuf=%p\n",
    ...
    if (!rga_ok)
    {
        ...
        if (raw_to_bgr_mat(const_cast<void *>(pSrcData), srcWidth, srcHeight, srcHStride, srcVStride, srcFmt, bgr))
        {
    ...
    bool logic_disp_fresh = false;
    {
        ...
        auto now_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch())
                .count();
        result_age_ms =
            static_cast<int64_t>(now_ms) - static_cast<int64_t>(g_pCtrl->channels_state[chnId].last_result_ts_ms);
        logic_disp = g_pCtrl->channels_state[chnId].logic_display_frame; /* 浅拷贝(引用计数), O(1) */
        logic_disp_fresh =
            !logic_disp.empty() && (static_cast<int64_t>(now_ms) -
    ...
    rp.roi_zones = &cs.roi_zones; /* 多 ROI: 全部区域(模型坐标系), render_overlays
                                     逐个绘制 */
    rp.results = &disp_results;
    rp.draw_cmds = &disp_draw_cmds;
    /* logic 用 display_canvas() 拦截了整帧 → 用它(640×640 BGR)当显示底图(缩放到
     * tile)，覆盖实时帧。 仅在新鲜(1s内)生效；陈旧则保留实时帧，防 logic
     * 停更后画面冻结。draw_cmds/检测框照旧叠加其上。*/
    if (logic_disp_fresh)
     ...
     * 本通道 swap_rb=1 时故意跳过这步：GTK 把 BGR 当 RGB 解析 → 屏幕上 R/B
     * 互换显示 （仅影响显示；推理/上报仍用正常 BGR，不受影响）。 */
