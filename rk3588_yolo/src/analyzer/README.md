...
**步骤一：** 在 `channel_logic.cpp` 底部实现逻辑函数：
...
static void logic_my_alarm(ChannelContext *ctx) {
    ...
    for (auto &r : ctx->results) {
        ...
        // 上报告警（冷却 5 秒）
        if (ctx->timestamp_ms - s.last_alarm_ms > 5000) {
...
**步骤二：** 在 `channel_logic_init()` 中注册：

```cpp
void channel_logic_init() {
    // ...已有注册...
    g_logic_map["logic_my_alarm"] = logic_my_alarm;  // ← 新增
}
```
