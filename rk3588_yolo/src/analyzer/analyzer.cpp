...
void analyzer_wake_display_threads(void)
{
    for (int channel_id : g_display_channel_ids)
    {
        pthread_mutex_lock(&g_disp_queues[channel_id].mtx);
        pthread_cond_broadcast(&g_disp_queues[channel_id].cv);
        pthread_mutex_unlock(&g_disp_queues[channel_id].mtx);
    }
}
...
void analyzer_destroy_display_queues(void)
{
    for (int channel_id : g_display_channel_ids)
    {
        pthread_mutex_destroy(&g_disp_queues[channel_id].mtx);
        pthread_cond_destroy(&g_disp_queues[channel_id].cv);
        g_disp_queues[channel_id].pool.deinit(); /* 释放三槽帧缓冲 */
    }
    g_display_channel_ids.clear();
