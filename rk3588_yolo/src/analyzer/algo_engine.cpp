...
{
    if (type == "yolov8_pose") return std::make_shared<YoloPose>(model_path, core_mask, obj_thresh, nms_thresh);
...
void *worker_thread_func(void *arg)
{
    ...
    while (g_algo.running && !g_algo.chn_reload_stop[chnId])
    {
        ...
        float lock_wait_ms = std::chrono::duration<float, std::milli>(
        ...
        if (task.src_buf && task.src_buf->handle != 0 && model_fd >= 0)
        {
            ...
            bool rga_ok = rga_convert_resize_handle(
                task.chnId, *task.src_buf,
                model_fd,
