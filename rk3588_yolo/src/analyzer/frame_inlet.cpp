 ...
 * @brief 视频帧入口
 *
 * videoOutHandle 是 GStreamer appsink
 * 回调调用的唯一帧入口，每解码一帧就调用一次。
 *
 * 每帧做三件事（顺序固定）：
 *   1. FPS 节流 + 交错调度（phase-offset）：按 max_fps
 * 决定本帧是否"处理"（推理通道与传统算法通道都限频）
 *   2. RGA 转换：NV12 → BGR 640×640（convertToYoloInput；仅"处理"的帧才转）
 *   3. 分流：
 *      - 推理通道(infer_enable 且配置了模型)：algorithm_process_mat → TaskQueue
 * → infer_worker
 *      - 传统算法通道(infer_enable=false)：同步调 process_channel_results（持
 * g_process_mtx，ctx->results 为空）
...
/*======================== 节流计时重置（供 analyzer_channel_offline/online
 * 调用）========================*/
...
int videoOutHandle(char *imgData, ImgDesc_t imgDesc)
{
    if (pause_ctrl::is_paused())
        return 0;
    if (imgDesc.chnId < 0 || imgDesc.chnId >= MAX_CHANNEL_NUM)
        return -1;
    ...
    int infer_enabled = 0;
    {
        ...
        if (ch < app_ctrl_get_chn_nums())
            infer_enabled = config_utils::is_channel_infer_enabled(g_pCtrl->config.channels[ch]) ? 1 : 0;
        pthread_rwlock_unlock(&g_pCtrl->mtx);
    }

    /* ---- FPS 节流（phase-offset 交错调度）----
     * 对推理通道与传统算法通道(infer_enable=false)都生效：后者也要按 max_fps
     * 限频跑 logic (需要 640 BGR 帧)，而不是每解码帧都跑。是否真正进 NPU 另由
     * infer_enabled 单独控制。*/
    int will_process = 0;
    ...
    int64_t current_frame_seq = 0;
    {
        ...
        /* 记录真实解码源分辨率：推理通道的 ROI 缩放(result_dispatch →
         * process_channel_results) 靠它把旧像素格式 ROI 从源像素缩到模型 640
         * 坐标系；否则 src_w 停在 0、roi_zones 不缩放，
         * 判定永远落在框外(画面只见显示用黄框、判定却 CLEAR)。(归一化 ROI
         * 加载即是模型坐标, 不受此影响)*/
     ...
     * 与推理通道一样按 max_fps 限频；raw_frame.model_input_mat 为 640
     * BGR(本帧转换或历史兜底)， 使 ctx->frame 可用于传统视觉(HSV/帧差等)；不进
     * NPU、ctx->results 为空、ctx->infer_enabled=0。*/
