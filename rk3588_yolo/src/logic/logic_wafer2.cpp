 ...
 * logic_wafer2 —— 晶圆擦拭轨迹分段可视化 (src/logic/visualize_trajectories.py
 * 移植)
 *
 ...
 *   ① 最高分 wafer 框 → 5
 * 步法提取晶圆椭圆(裁剪→灰度→高斯模糊→暗区二值→开闭运算→
 *      碎片轮廓→凸包→fitEllipse), 面积突变剔除 + EMA 时序平滑(同脚本
 * extract_wafer_ellipse); ② 最高分刮片框中心追加到轨迹(t = 距本轮起点秒数); ③
 * 覆盖率: 500×500 归一化掩码累计 圆点+连线(刮片宽度固定16px), 单位圆内占比 (同
 * compute_coverage_from_ellipse; 脚本用全视频中值椭圆做基准,
 * 实时版用当前平滑椭圆); ④ 轨迹分段(segment_trajectory 逐行移植):
 * 时间间隙预切(>=0.15s) → 折返率判 横擦(linear)/环擦(spiral) → 长直线段按重平滑
 * PCA 角度突变再切 → 相邻短段合并 → 边界 spiral 复判; 每新增 5
 * 个点重算一次(脚本为全轨迹离线一次性计算); ⑤ 叠加: 红色虚线椭圆+「覆盖基准」 →
 * 绿色半透明覆盖带 → 当前点(段色圆点+白圈) → 最近 3s 分段轨迹 →
 * 顶部图例(覆盖面积/刮片宽度/横擦·竖擦·环擦 模板槽位 ▶✓○)。
 *
 * 与脚本的差异(实时化所需):
 *   - 脚本跑完整段视频后回放绘制; 本逻辑边采集边算, 分段结果随轨迹增长而细化;
 *   - 覆盖带视觉效果用全程轨迹半透明粗折线近似(脚本是掩码反变形回椭圆内,
 * 视觉接近);
 *   - 刮片消失超 5s 视为一轮结束并自动复位(脚本单视频离线, 无需复位;
 * 循环流必须有);
 *   - YOLO 检测框由框架固定绘制(脚本不画), 此处仅改色: 刮片绿/晶圆灰。
 *
 * 参数暂写死(来源: 脚本命令行默认值 + config.yaml 中脚本实际用到的项):
 *   dark_threshold=105(--dark-thresh 默认; 注: config.yaml
 * roi.dark_threshold=80 脚本并未使用), smooth_alpha=0.05(roi.smooth_alpha),
 * trail_sec=3.0(--trail-sec), 刮片宽=16px(覆盖计算固定值),
 *   mask_res=500。config.yaml 的 tracking/sop 两节属其它脚本,
 * visualize_trajectories.py 不读取; detector
 * 节(conf=0.25/iou=0.45/imgsz=640)对应通道模型配置 → 该通道 obj_thresh 请设
 * 0.25。
 * ==========================================================================*/
...
struct W2Ellipse
{
    double cx = 0, cy = 0, a = 0, b = 0, angle = 0;
}; /* (a,b)=全轴长, 同脚本 */
...
/* 滑动窗折返率: 步进方向变化 >150° 记一次折返, 窗口 [t-w/2, t+w/2] 内
 * 折返数/时长。 窗内点数 <5 或时长 <=0.3s → NAN(无效)。对应脚本
 * compute_reversal_rate(双指针等价加速)。 */
static std::vector<double> w2_reversal_rate(const W2Point *p, int n, double window_sec)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        while (hi < n && p[hi].t <= wh)
        {
            revs += rev[hi];
            ++hi;
        }
        while (lo < hi && p[lo].t < wl)
        {
            revs -= rev[lo];
            ++lo;
        }
...
static void w2_pca_angle_smoothed(const W2Point *p, int n, int half_window, int kernel, std::vector<double> &out_ts,
                                  std::vector<double> &out_ang)
{
    out_ts.clear();
    out_ang.clear();
...
static std::vector<W2Seg> w2_segment_trajectory(const std::vector<W2Point> &pts)
{
    ...
    for (int i = 1; i < n; ++i)
    {
        ...
        {
            if (i - 1 - seg_start >= 3)
                time_segs.push_back({seg_start, i - 1});
            seg_start = i;
        }
    }
    if (n - 1 - seg_start >= 3)
        time_segs.push_back({seg_start, n - 1});
    if (time_segs.empty())
        time_segs.push_back({0, n - 1});
    ...
    for (const auto &tspan : time_segs)
    {
        ...
        if (sub_n < 10)
        {
            all.push_back({ts_a, ts_b, W2_SHORT});
            continue;
        }
        ...
        if ((int)valid_rates.size() < 10)
        {
            all.push_back({ts_a, ts_b, W2_LINEAR});
            continue;
        }
        ...
        for (double t0 = sub[first_valid].t; t0 < sub[last_valid].t; t0 += block_sec)
        {
            ...
            if ((int)rs.size() >= 3)
                blocks.push_back({w2_median(rs), bs, be, 0});
        }
        ...
        for (auto &b : blocks)
            b.phase = (b.rate >= thr) ? W2_LINEAR : W2_SPIRAL;

        /* 相位变化处分界 → 块组 → rr_segs */
        std::vector<int> bounds;
        bounds.push_back(0);
        for (int i = 1; i < (int)blocks.size(); ++i)
            if (blocks[i].phase != blocks[i - 1].phase)
                bounds.push_back(i);
        ...
        for (const auto &rs : rr_segs)
        {
            ...
            for (size_t k = 0; k < v_ts.size(); ++k)
                if (v_ts[k] >= sub[a].t && v_ts[k] <= sub[b].t)
                {
                    sa.push_back(v_ang[k]);
                    st.push_back(v_ts[k]);
                }
            ...
            {
                if (st[i] < search_lo || st[i] > search_hi)
                    continue;
                const double before =
                    w2_median(std::vector<double>(sa.begin() + std::max(0, i - half_w), sa.begin() + i));
                const double after =
                    w2_median(std::vector<double>(sa.begin() + i, sa.begin() + std::min(n_ang, i + half_w)));
                double diff = std::fabs(after - before);
                if (diff > 180.0)
                    diff = 360.0 - diff;
                if (diff > best_shift && diff >= 30.0)
                {
                    best_shift = diff;
                    best_split_t = st[i];
                }
    ...
    /* ---- ⑤ 边界 spiral 复判: a)往复直线 lin>0.70 且折返率>0.5; b)平推扫掠
     * lin>0.55 且局部角度标准差<40° 且平均步长>10px → 改判 linear ---- */
    for (auto &sgm : merged)
    {
        if (sgm.phase != W2_SPIRAL)
            continue;
        ...
        const std::vector<double> r2 = w2_reversal_rate(sp, m, std::min(1.5, pts[sgm.b].t - pts[sgm.a].t));
        double rsum = 0;
        int rcnt = 0;
        for (double v : r2)
            if (!std::isnan(v))
        ...
        if (lin > 0.70 && avg_r > 0.5)
            sgm.phase = W2_LINEAR;
        else if (lin > 0.55 && ang_std < 40.0 && mean_step > 10.0)
            sgm.phase = W2_LINEAR;
    }
    return merged;
}
 ...
 */
static bool w2_extract_ellipse(const cv::Mat &frame, const cv::Rect &bbox, int dark_threshold, double alpha,
                               bool have_last, W2Ellipse &e)
{
    ...
    for (const auto &c : contours)
        if (cv::contourArea(c) > 100.0)
            for (const auto &q : c)
                all_pts.emplace_back(q.x + x1, q.y + y1);
    if ((int)all_pts.size() < 5)
        return have_last;
...
static void logic_wafer2(ChannelContext *ctx)
{
    ...
    if (!*ctx->state)
        *ctx->state = std::make_shared<Wafer2State>();
    ...
    static const cv::Scalar W2_COLORS[5] = {
        /* 分段调色板(BGR), 同脚本 SEGMENT_COLORS */
    ...
    for (auto &r : *ctx->results)
    {
        if (r.class_id == 0)
        {
            if (!scraper || r.score > scraper->score)
                scraper = &r;
        }
        else if (r.class_id == 1)
        {
            if (!wafer || r.score > wafer->score)
                wafer = &r;
        }
    }
    if (scraper)
        scraper->box_color = cv::Scalar(0, 255, 0);
    if (wafer)
        wafer->box_color = cv::Scalar(160, 160, 160);

    /* ---- ② 晶圆椭圆(当前框, 缺检沿用上次框, 同脚本 current or last_wafer_bbox)
     * ---- */
    if (wafer)
    {
        s.last_bbox = wafer->box;
        s.have_bbox = true;
    }
    if (s.have_bbox)
        s.have_ellipse =
            w2_extract_ellipse(*ctx->frame, s.last_bbox, DARK_THRESHOLD, EMA_ALPHA, s.have_ellipse, s.ellipse);
        ...
        if ((int)s.pts.size() < MAX_PTS)
        {
            ...
            if (s.have_ellipse)
            {
                ...
                const int mx = std::min(
                    std::max((int)((c.x - s.ellipse.cx) / rx * (MASK_RES / 2.0) + MASK_RES / 2.0), 0), MASK_RES - 1);
                const int my = std::min(
                    std::max((int)((c.y - s.ellipse.cy) / ry * (MASK_RES / 2.0) + MASK_RES / 2.0), 0), MASK_RES - 1);
    ...
    /* ---- ⑤ 分段(节流重算; 脚本为离线一次性) ---- */
    if ((int)s.pts.size() >= 2 && (s.segs.empty() || (int)s.pts.size() - s.pts_at_seg >= SEG_EVERY_PTS))
    {
    ...
    if (s.pts.size() >= 2) /* 覆盖带: 全程轨迹半透明绿粗折线 */
    {
        ...
        for (const auto &q : s.pts)
            tp.emplace_back((int)q.x, (int)q.y);
    ...
    {
        if (s.pts[i].t < t_now - TRAIL_SEC)
            break;
        const int k = seg_idx_of(i);
        if (k < 0 || k != seg_idx_of(i - 1))
            continue; /* 跨段不连线(同脚本) */
        draw_line(ctx, cv::Point((int)s.pts[i - 1].x, (int)s.pts[i - 1].y), cv::Point((int)s.pts[i].x, (int)s.pts[i].y),
                  W2_COLORS[k % 5], 2);
    }
    ...
    /* 模板槽位(同脚本 TEMPLATE): 检出的 linear 段依次填 横擦/竖擦, spiral 段填
     * 环擦 */
    struct Slot
    {
        const char *name;
