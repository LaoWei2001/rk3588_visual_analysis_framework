 ...
 *
 * 背景：AI
 * 视觉管理"人工用工具擦拭晶圆盘"工序。模型类别：0=cleanwiper(工具尖端),
 * 1=wafer(晶圆盘)。 计算区域 = wafer(class 1) 检测框本身(动态)，不用手画 ROI。
 * 流程(状态机)：
 *   - 工序开始：cleanwiper 中心进入 wafer 框并持续 >= t_start 秒 →
 * 开始(计时/记轨迹/算覆盖)。
 *     确认期滤除手短暂进入、反光/闪光误检；期间不画不计。
 *   - 进行中：每帧把工具中心连成轨迹，按 line_width
 * 拓宽成"扫过面积带"(单色半透明铺底， 不再实时分类/染色)；左上实时显示
 * 覆盖率(以实时 wafer 框为基准)/用时/状态。
 *   - 工序结束：工具离开区域或消失持续 >= t_end 秒 → 结算：①
 * 覆盖率(以"结束那一刻 wafer 框" 为基准) >= coverage_threshold；②
 * 把整条轨迹交给 wafer_detect_missing(点集, required) 离线
 *     分析有无漏动作(横/纵/弧)。两者都过 →
 * 合格；否则右上角红字报警(驻留到下一工序)。
 * 不上报；坐标全部在模型输入坐标系(640)，与检测 box 一致。可调参数见
 * logics.json。
 */
...
/* 对一段连续点 p[0..m-1] 算特征：PCA(线性) + 逐步行程(横/纵) + Kåsa
 * 圆拟合(残差/角程)
...
static WaferMotionFeat wafer_motion_features(const cv::Point *p, int m)
{
    ...
    if (std::fabs(cdet) > 1e-6)
    {
        ...
        if (R2 > 1e-6)
        {
            ...
            f.circ_R = R;
            double se = 0.0, prevphi = 0.0, arc = 0.0;
            bool hp = false;
            for (int i = 0; i < m; ++i)
            {
    ...
    for (int i = 0; i < m; ++i)
        proj[i] = static_cast<float>((p[i].x - mx) * ux + (p[i].y - my) * uy);
...
static int wafer_window_label(const cv::Point *p, int m)
{
    ...
    if (!f.valid || f.extent < 3.0)
        return 0;
    /* 圆弧：圆拟合残差小 + 角程足够 + 非共线 + 半径合理 */
    if (f.circ_resid < 0.12 && f.arc_path >= 1.0 && f.linearity < 0.93 && f.circ_R > 4.0)
        return 3;
    /* 直线往复门控(确认是"擦"而非噪声漂移) */
    ...
    if (f.trav_dx >= f.trav_dy * 1.15)
        return 1;
    if (f.trav_dy >= f.trav_dx * 1.15)
        return 2;
    return 0;
}

/* 轨迹点(坐标 + 编号)——结算分析函数的输入元素 */
struct WaferTrajPoint
{
    cv::Point pt;
    int id;
};

/* 解析 required_actions(逗号/空格/斜杠/分号分隔；支持 横擦/纵擦/圆弧擦 或
 * h/v/arc) → 需校验的动作 id 列表(1=横,2=纵,3=弧)；空串 → 空列表(不校验)。 */
static std::vector<int> wafer_parse_required(const std::string &req)
{
    ...
    auto flush = [&](std::string t) {
        ...
        t = t.substr(a, b - a + 1);
        if (t.find("\xE6\xA8\xAA") != std::string::npos || t == "h" || t == "H")
            need_h = true; /* 横 */
        else if (t.find("\xE7\xBA\xB5") != std::string::npos || t == "v" || t == "V")
            need_v = true; /* 纵 */
 ...
 * 方法：对整条轨迹滑动窗口逐段分类(圆拟合判弧 /
 * 逐步行程判横纵)，统计各动作出现的
 *      窗口数，达到支持度阈值即认定该动作出现过，最后求缺失。 */
static std::vector<int> wafer_detect_missing(const std::vector<WaferTrajPoint> &pts, const std::vector<int> &required)
{
    if (required.empty())
        return {};

    std::vector<cv::Point> xy;
    xy.reserve(pts.size()); /* 按编号取有序坐标(轨迹本就有序) */
...
static void logic_wafer(ChannelContext *ctx)
{
    ...
    if (!*ctx->state)
        *ctx->state = std::make_shared<WaferState>();
    ...
    for (auto &r : *ctx->results)
    {
        if (r.class_id != 1)
            continue;
        if (r.score > best_region_score)
        {
            best_region_score = r.score;
            s.region_box = r.box;
            s.have_region = true;
        }
    }
    ...
    {
        if (r.class_id != 0)
            continue; /* 0=cleanwiper(按下标，比类名稳) */
        found_in_frame = true;
        r.box_color = cv::Scalar(0, 255, 0);
        const cv::Point c = r.box_center();
        if (region.contains(c) && r.score > best_score)
        {
            best_score = r.score;
            new_pt = c;
            have_new_pt = true;
    ...
    if (!s.op_active)
    {
        ...
        else if (now - s.region_last_ms > region_grace_ms)
            s.enter_ms = 0;
    ...
    if (!s.trajectory.empty())
    {
        ...
        if (region_area > 0.0)
            ratio = cv::countNonZero(s.swept_mask(region)) / region_area;
    }
    ...
    if (s.op_active)
    {
        ...
        if (now - s.region_last_ms >= t_end_ms)
        {
            ...
            if (s.last_coverage + 1e-3f < cov_thresh)
            {
                pass = false;
                char b[64];
                snprintf(b, sizeof(b), "Coverage %.0f%% < %.0f%%", s.last_coverage, cov_thresh);
                msg = b;
            }
            ...
            if (!required.empty())
            {
                std::vector<WaferTrajPoint> pts;
                pts.reserve(s.trajectory.size());
    ...
    /* 左上角：覆盖率 / 状态+动作 / 用时 */
    const float disp_cov = s.op_active ? static_cast<float>(ratio * 100.0) : (s.have_result ? s.last_coverage : 0.0f);
    char l1[80];
    ...
    if (s.op_active && have_new_pt)
    {
        snprintf(l2, sizeof(l2), "TRACKING pts:%zu", s.trajectory.size());
        l2c = cv::Scalar(0, 255, 0);
    }
    else if (s.op_active)
    {
        snprintf(l2, sizeof(l2), "ENDING %.1f/%.1fs", end_elapsed, t_end);
        l2c = cv::Scalar(0, 200, 255);
    }
    else if (s.enter_ms != 0)
    {
        float dw = (now - s.enter_ms) / 1000.0f;
        snprintf(l2, sizeof(l2), "CONFIRMING %.1f/%.1fs", dw, t_start);
        l2c = cv::Scalar(0, 165, 255);
    }
    else if (found_in_frame)
    {
        snprintf(l2, sizeof(l2), "wiper out of region");
        l2c = cv::Scalar(180, 180, 180);
    }
    else
    {
        snprintf(l2, sizeof(l2), "idle");
        l2c = cv::Scalar(180, 180, 180);
    }
    ...
    if (s.op_active)
    {
        float run = (now - s.op_start_ms) / 1000.0f;
        snprintf(l3, sizeof(l3), "Time: %.1fs", run);
    }
    else if (s.have_result)
        snprintf(l3, sizeof(l3), "Last: %.1fs", s.last_op_sec);
    else
        snprintf(l3, sizeof(l3), "Time: 0.0s");
