...
在一块 RK3588 上同时接入多路视频（RTSP / 本地文件 / USB），用 NPU 跑 YOLO 检测/语义分割/姿态识别，按「每通道一段自行编排的业务逻辑」做入侵、计数、跌倒识别等视觉分析并支持上报服务器/dify，全程可在网页上拖拽配置、热重载参数、看实时画面。本项目架构的设计既适合二次开发时的调试也适合实际项目的开发与部署。上层的逻辑编排部分与底层代码解耦，二次开发者只需要关注逻辑的实现，不用关注底层的视频取流，解码，NPU推理，多线程调度等内容。本项目配有基础的教程，开发者经过短暂的学习即可自行编写出有趣的功能。docs文件夹中也配有看具体的模块说明供开发者参考，也可直接作为供大模型参考的提示词。

如有任何疑问或意见可随时联系作者：Sunny_Wei, Email:1927096839@qq.com

祝编程愉快！ Happy coding! :)

[![Hits](https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fgithub.com%2FLaoWei2001%2Frk3588_visual_analysis_framework&count_bg=%2379C83D&title_bg=%23555555&icon=&icon_color=%23E7E7E7&title=hits&edge_flat=false)](https://hits.seeyoufarm.com)
---
...
- **支持传统cv算法**：可对视频帧直接处理后显示在视频窗口，便于调试。
...
`build.sh` 产出的 `dist/` 也能脱离web控制台独立运行（需先准备一份 `config.json`，最简单是用控制台生成后导出，或参考 `docs/` 或其他已有的json配置文件自己写）：
...
- **无法在web端或vscode启动板端显示器的视频画面**：需要从板端命令行直接启动1次程序，而不是从ssh连接启动，之后便可以在web或者vscode启动连接到板端的HDMI显示器的视频画面了，具体原因未知。
...
- **长时间高NPU负载运行之后，CPU占用率大幅波动，NPU占用率达不到90%**：原因未知，可能是温度过高触发了操作系统的保护机制。
