...
在一块 RK3588 上同时接入多路视频（RTSP / 本地文件 / USB），用 NPU 跑 YOLO 检测/分割/姿态或者传统cv视觉算法，按「每一路分配一个自行编排的算法逻辑」做入侵、人员计数、姿态识别等视觉分析并支持上报服务器/Dify，全程可在网页上拖拽配置、热重载参数、看实时画面。另外本项目的架构设计十分便于后续的二次开发和部署，开发者只需关注上层逻辑的实现，而底层的多路视频取流，解码，缩放，NPU推理，多线程调度等内容已与上层逻辑解耦。项目目录中包含了基础的教程，经过学习，短时间内就可以编写出有趣的功能。

如有任何疑问或意见可随时联系作者：Sunny_Wei 1927096839@qq.com
祝编程愉快！Happy coding! :)
