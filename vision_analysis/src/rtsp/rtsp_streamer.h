 ...
 *   它是 RGB、disp_width×disp_height、含全部叠加框(检测框/自定义绘制/ROI/FPS)的整幅画面，
 *   与接显示器看到的内容完全一致。
 *
 * 设计要点:
 ...
 *   - 可通过 rtsp_encoder="hw" 强制硬编；不支持强制软编。
 *   - 推帧用 g_signal_emit_by_name(appsrc,"push-buffer",...)，与采集端 pull-sample 风格一致，
 *     不需要链接 gstreamer-app。
 *
 * 接入 (main.cpp):
 *   1. 分配 g_disp 缓冲的条件需含 enable_rtsp:
 *        if (app_ctrl_get_enable_disp() || app_ctrl_get_enable_rtsp()) { ... dispBufferMap ... }
 *   2. pipeline_submit_frame 在 enable_display 或存在 RTSP 客户端时推显示队列 (frame_inlet.cpp)。
