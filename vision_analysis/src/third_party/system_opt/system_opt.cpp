...
注意：
    ...
    * 如果你希望使用 wait () 或 waitpid () 对子进程收尸，那么你必须在调用前(事实上是fork
()前)将SIGCHLD信号置为SIG_DFL处理方式。 调用后(事实上wait()/waitpid()后)再将信号处理方式设置为从前的值。 调用建议：
...
static int32_t SYSTEM(const char *cmdstring)
{
    ...
    //         -保证子进程先运行，在调用 exec 或 exit 之前与父进程数据是共享的。父进程在子进程调用 exec 或 exit
...
注意：
    ...
    * system () 函数首先 fork () 一个子进程执行 command 命令，执行完后 system () 函数会使用 waitpid ()
函数对子进程进行收尸。
********************************************************************/
