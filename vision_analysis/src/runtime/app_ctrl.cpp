...
extern "C" void *config_monitor_thread_func(void *arg)
{
    ...
    while (!ctrl->config_monitor_exit.load())
    {
        ...
        {
            if (inference_reload_channel_model(new_cfg.channels[config_index].id, new_cfg.channels[config_index]))
            {
        ...
        for (const auto &channel : base_applied_cfg.channels)
        {
            printf("[ConfigMonitor] Updating channel %d model filters (%zu configured)\n", channel.id,
                   channel.models.size());
            inference_update_thresh(channel.id, channel);
            inference_update_detect_classes(channel.id, channel);
        }
        inference_update_queue_size(base_applied_cfg.queue_size);

        if (!pipeline_publish_runtime_snapshot(base_applied_cfg, base_generation, logic_changed_channels,
         ...
         * 复用 DecChannel::init() + busListen 的已有基础设施, 与断流重连共用重连逻辑。 */
        {
            ...
            for (auto &sw : stream_switches)
            {
                ...
                else
                {
                    ...
                    pipeline_channel_offline(chnId);
                }
        ...
        if (stream_runtime_changed)
        {
            ...
            if (!pipeline_publish_runtime_snapshot(applied_cfg, generation))
            {
...
{
    return inference_get_infer_fps(chnId);
}
