 ...
 * @brief pipeline 模块内部共享接口
 *
 * 仅供 pipeline/ 目录下各 .cpp 文件相互引用，不对外暴露。
 * 外部模块请使用 pipeline_runtime.h。
 *
 * 各文件职责速查:
 ...
 * 问题背景
 * --------
 * 原来 DisplayTask 用 std::vector<uint8_t> data 存储完整帧像素，
 * pipeline_submit_frame 的 data.assign() 在持 DisplayQueue::mtx 期间执行一次
 ...
 *   slots[mid_idx]    — 最新就绪帧（handoff 槽，publish/swap 时持 DisplayQueue::mtx）
...
struct DisplayFramePool
{
    ...
    /** 释放三槽内存，由 pipeline_destroy_display_queues 调用。*/
    void deinit()
    {
        for (auto &s : slots)
        {
 ...
 * @brief 显示任务（仅元数据）。
 *
 * 帧像素数据由 DisplayQueue::pool（DisplayFramePool）管理，
...
struct DisplayQueue
{
    ...
    DisplayTask task;      /* 元数据：6 个整数 */
    DisplayFramePool pool; /* 帧像素：三槽预分配缓冲 */
};
