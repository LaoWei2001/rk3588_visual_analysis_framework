...
int algorithm_init(const AppConfig &cfg)
{
    ...
    for (size_t i = 0; i < cfg.channels.size() && i < MAX_CHANNEL_NUM; ++i)
    {
        ...
        if (channel_id < 0 || channel_id >= MAX_CHANNEL_NUM) continue;
        const size_t active_models = std::count_if(
            channel.models.begin(), channel.models.end(),
                ...
                if (active_models.size() > 1)
                {
                    ...
                    for (size_t model_index = 0; model_index < active_models.size(); ++model_index)
                    {
                        ...
                        const int core_id = (spec.npu_core >= 0 && spec.npu_core <= 2)
                            ? spec.npu_core : (auto_model_instances++ % 3);
                        ...
                        log_printf_threadsafe(
                            "[Algo] Created composite child %zu/%zu for ch%d: %s core=%d\n",
    ...
    if (!g_algo.running.load())
    {
        ...
        for (int i = 0; i < MAX_CHANNEL_NUM; ++i)
        { g_fps[i].init(now); g_perf[i].init(now_ms); }
...
void algorithm_update_thresh(int chnId, const ChannelConfig &config)
{
    ...
    const size_t active_count = std::count_if(
        models.begin(), models.end(), [](const ChannelModelConfig &model)
        { return model.enable && !model.model_path.empty() && !model.model_type.empty(); });
    if (active_count > 1)
    {
...
{
    if (chnId < 0 || chnId >= MAX_CHANNEL_NUM || !g_algo.running) return false;
    ...
    {
        log_printf_threadsafe(
            "[Algo] ch%d worker topology change rejected (%d -> %d); restart required\n",
            chnId, worker_count, requested_worker_count);
        pthread_rwlock_unlock(&g_algo.dispatch_mtx);
        return false;
    }
    const std::vector<std::shared_ptr<ModelBase>> old_models =
        g_algo.models_per_chn[chnId];
    const float old_obj_thresh =
        g_algo.obj_thresh[chnId].load(std::memory_order_relaxed);
    const float old_nms_thresh =
        g_algo.nms_thresh[chnId].load(std::memory_order_relaxed);
    ...
    auto restart_workers = [&](const std::vector<std::shared_ptr<ModelBase>> &new_models) -> bool
    {
        ...
        for (int w = worker_start; w < worker_start + worker_count; ++w)
        {
            ...
            const int create_rc = pthread_create(&g_algo.worker_tids[w], nullptr,
        ...
        for (int t = 0; t < worker_count; ++t)
        {
            ...
            if (active_models.size() > 1)
            {
                ...
                for (size_t model_index = 0; model_index < active_models.size(); ++model_index)
                {
                    const auto &spec = active_models[model_index];
                    const int core_id = (spec.npu_core >= 0 && spec.npu_core <= 2)
                        ? spec.npu_core : ((worker_start + t + static_cast<int>(model_index)) % 3);
                    auto child = create_model(spec.model_type, spec.model_path, spec.label_path,
                                              core_masks[core_id], spec.obj_thresh, spec.nms_thresh);
                    if (!child)
                        throw std::runtime_error("unsupported model_type: " + spec.model_type);
    ...
    {
        log_printf_threadsafe(
            "[Algo] Model reload incomplete for ch%d (%zu/%d workers); restoring old model\n",
