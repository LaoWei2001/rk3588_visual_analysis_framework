 ...
 *   → algorithm_wait_result (阻塞等待) → algorithm_take_results (原子取惰性帧引用+框)
 *   → process_channel_results → invoke_channel_logic → event_report
 *
 * 帧匹配保证（此文件不涉及）：
 *   algorithm_take_results 在同一把锁下原子取出 LazyVideoFrame 与检测框，
...
extern "C" void *dispatch_worker_thread(void *arg)
{
    ...
    while (g_dispatch_running.load())
    {
        ...
        const int has_new = algorithm_take_results(chnId, current_results, lazy_frame, result_frame_id,
