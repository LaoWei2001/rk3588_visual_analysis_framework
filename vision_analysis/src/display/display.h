...
/* RenderParams 定义在 rkvision/channel_context.h（DrawCommand / AlgoResult 也在那里）
 * 这里只做前置声明，避免循环 include：
 *   rkvision/channel_context.h → display.h → rkvision/channel_context.h */
struct RenderParams;
 ...
 *
 * RenderParams 完整定义在 rkvision/channel_context.h，调用方 .cpp 须先 include rkvision/channel_context.h。
 */
