...
static void *image_worker(void *)
{
    while (true)
    {
        ...
        try
        {
            if (job.need_annotated && job.overlay_mode != "none" && !job.annotated.empty())
            {
...
EventReportResult report_event(ChannelContext *ctx, const EventRequest &input)
{
    ...
    if (media_flags & EVENT_MEDIA_IMAGE)
    {
        ...
        {
            update_media_state(dir, "image", "failed", "current frame is empty");
            media_errors.push_back("image: current frame is empty");
        }
        else
        {
