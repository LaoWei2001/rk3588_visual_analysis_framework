...
flowchart TD
    ...
    L --> M["计算变化像素占有效像素的比例"]
    M --> O{"当前状态"}
    O -->|静止| P{"变化像素占比<br/>是否达到统一阈值"}
    P -->|未达到| P0["保持静止并清除运动候选计时"]
    P -->|已达到| Q{"是否连续达到运动确认时间"}
    Q -->|否| Q0["保持运动候选计时"]
    Q -->|是| R["发布行车运动"]
    O -->|运动| S{"变化像素占比<br/>是否低于统一阈值"}
...
处于静止状态时，只有 $R_{pixel}$ 达到 `change_ratio` 并连续保持 `moving_confirm_sec`，系统才切换为运动。处于运动状态时，只有 $R_{pixel}$ 低于同一个 `change_ratio` 并连续保持 `still_confirm_sec`，系统才切换回静止。

进入运动与进入静止共用一个变化比例阈值，但分别使用运动确认时间和静止确认时间，仍可通过时间滤波抑制瞬时抖动。

### 5.2 可视化与调参

运动检测窗口同时常驻显示行车状态、实时有效变化像素百分比、统一变化比例阈值、运动/静止实时累计确认时间及其设定阈值，以及帧差灰度阈值。高斯核固定为 5，亮度归一化固定开启，不再作为 Web 配置项或画面状态文字。二值帧差图中发生有效变化的每个像素都会以半透明橙色叠加到预览画面对应位置，不再显示分块边框。现场人员可以直接观察“具体哪些像素产生了有效帧差”和“当前变化像素占比为何达到或未达到状态阈值”，降低现场阈值调试对算法开发人员的依赖。

Web 按钮可循环选择并调整帧差灰度阈值、统一变化比例以及运动/静止确认时间。全部参数始终可见，当前选择项只以黄色高亮表示它是增减按钮的操作目标。相邻帧基准会在首帧和图像尺寸变化时自动建立，不再提供手动重建按钮。
...
安全帽模型只需要训练两类目标：`person` 和 `helmet`，不需要单独标注“未戴安全帽”。系统首先使用人员框底边中心判断人员是否位于装卸区域 `loading_zone`，然后对区域内每个人员，检查其检测框是否与任一安全帽检测框存在交集：存在交集即认为该人员已佩戴安全帽，否则计为未戴安全帽。

匹配成功的人员框显示为绿色；没有与任何安全帽框相交的人员框显示为红色，并计入未戴安全帽人数。只要未戴安全帽人数大于零且持续达到违规确认时间，就发布 `helmet_alarm`；连续安全达到解除时间后才清除告警。
...
| 运动检测 | `change_ratio` | 运动和静止共用的变化比例阈值 | 用真实大车、小车启停视频记录变化比例，阈值应高于静止噪声且低于最慢运动时的变化比例 |
...
3. **阈值与时间状态机**：运动/静止共用变化比例阈值但使用不同确认时间；越界/安全、违规/解除也使用持续时间防止状态抖动。
