...
struct GlobalContext
{
    ...
    /**
     * @brief 本实例监控的稳定配置通道 ID 列表指针（只读）。
     *
     * 请通过 for_each_channel() 遍历，不要直接解引用（框架保证非空）。
     * 值对应 config.channels[].id；若配置中 channels 为空数组，框架会自动填充所有活跃通道。
     */
     ...
     *   if (s.frame.empty() || s.result_age_ms > 500) return;
     *
     * @param configuredId  config.channels[].id（不限于本实例的 channel_ids）
     */
     ...
     */
    int channel_has_target(int configuredId, const char *label, int max_age_ms = 200) const
    {
        return app_ctrl_has_target(configuredId, label, max_age_ms);
    }
     ...
     */
    int get_channel_target_count(int configuredId, const char *label, int max_age_ms = 200) const
    {
        return app_ctrl_get_target_count(configuredId, label, max_age_ms);
    }
