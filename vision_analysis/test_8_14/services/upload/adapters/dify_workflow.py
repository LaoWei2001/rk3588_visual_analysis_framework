...
class DifyWorkflowAdapter(DeliveryAdapter):
    ...
    def send(self, event_dir: str, event: Dict[str, Any], delivery: Dict[str, Any]) -> DeliveryResult:
            ...
            if response.status_code != 201:
                return DeliveryResult(
                    ...
                    terminal=(
                        ...
                        and not is_retryable_http_status(response.status_code)
                    ),
                )
            file_id = str(response.json().get("id", ""))
            if not file_id:
                return DeliveryResult(
                    False, "Dify file upload response has no id", response.status_code,
                    terminal=True,
                )
            value = {
        ...
        return DeliveryResult(ok, detail, response.status_code, response_body, terminal)
