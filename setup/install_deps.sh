...
#
#   bash setup/install_deps.sh              安装运行、C/C++ 编译环境并预构建 Web 前端
#   bash setup/install_deps.sh --runtime-only    只安装运行环境
#   bash setup/install_deps.sh --check      不联网、不修改系统，验收默认完整环境
#   bash setup/install_deps.sh --check --runtime-only   只验收运行环境
#   bash setup/install_deps.sh --skip-app-check  准备制包机时忽略从其他系统复制来的旧二进制
#
...
case "$SYSTEM_ID" in
    ubuntu)
        DEPENDENCY_MANIFEST="$PROJ/setup/offline/ubuntu/dependency_manifest.sh"
        ;;
    *)
        DEPENDENCY_MANIFEST="$PROJ/setup/offline/debian/dependency_manifest.sh"
        ;;
esac
...
# shellcheck source=setup/offline/debian/dependency_manifest.sh
...
PY
    ...
    if "$check_python" -m pip check; then
        ...
        check_fail "pip 报告依赖缺失或版本冲突；断网机请重新运行 setup/offline/debian/install_offline.sh。"
    fi
    ...
    if [ ! -f /usr/share/fonts/truetype/wqy/wqy-zenhei.ttc ] \
        ...
        && [ ! -f "$PROJ/projects/person_count/assets/fonts/overlay.ttf" ] \
        && [ ! -f "$PROJ/projects/person_count/assets/fonts/overlay.ttc" ] \
        && [ ! -f "$PROJ/projects/person_count/assets/fonts/overlay.otf" ]; then
...
check_application_binaries() {
    ...
    while IFS= read -r -d '' binary; do
        found=$((found + 1))
        check_one_binary "参考应用构建产物" "$binary" false
    done < <(find "$PROJ/projects/person_count/build" -name vision_analysis -type f -perm -u+x -print0 2>/dev/null || true)
    if [ -x "$PROJ/tools/first_net_config/first_net_config" ]; then
        found=$((found + 1))
        check_one_binary "首次网络配置程序" \
            "$PROJ/tools/first_net_config/first_net_config" false
    fi
