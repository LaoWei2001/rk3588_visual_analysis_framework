...
# setup/install_deps.sh 会安装运行、编译、制包、Python、Node.js 和前端依赖。
bash "$PROJECT_ROOT/setup/install_deps.sh" --skip-app-check
...
command -v dpkg-scanpackages >/dev/null 2>&1 \
    || { echo "[错误] setup/install_deps.sh 完成后仍缺少 dpkg-scanpackages。" >&2; exit 1; }
command -v dpkg-repack >/dev/null 2>&1 \
    || { echo "[错误] setup/install_deps.sh 完成后仍缺少 dpkg-repack。" >&2; exit 1; }
...
if [ "${OFFLINE_PREPARE_EMBEDDED:-false}" = true ]; then
    ...
    echo "     可执行: bash setup/offline/debian/create_bundle.sh"
fi
