...
# 等价的明确写法：bash setup/offline/debian/create_bundle.sh --full
```
...
该命令固定生成 `setup/offline/debian/output/source-update`，不会执行 C/C++ 编译、前端
...
完整源码中包含 `tools/first_net_config/`，需要时可在目标机自行编译。bundle 根目录不再额外放置
...
```text
create_bundle.sh --full        -> setup/offline/debian/output/full-bundle
create_bundle.sh --source-only -> setup/offline/debian/output/source-update
```
...
```bash
bash setup/offline/debian/detect_apt_dependencies.sh
```
...
保存后仍然只需重新运行 `bash setup/offline/debian/create_bundle.sh`。
...
安装完成后可按需执行 `bash setup/install_deps.sh --check` 诊断硬件、驱动和动态库。它是排障工具，
