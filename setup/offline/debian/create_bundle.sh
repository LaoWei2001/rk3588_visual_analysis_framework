...
#
#      输出目录：setup/offline/debian/output/full-bundle
...
#
#      简写：bash setup/offline/debian/create_bundle.sh source
#      输出目录：setup/offline/debian/output/source-update
...
#
#   3. 查看帮助
#      bash setup/offline/debian/create_bundle.sh --help
set -Eeuo pipefail
...
ENV_DISPLAY_PATH="setup/offline/debian"
...
APP_PROJECT="$PROJECT_ROOT/projects/person_count"
...
# 避免耗时制包结束后才发现源码不完整。
PLATFORM_MANAGER_REL="setup/install.sh"
...
required_project_files=(
    ...
    "tools/hardware/gpio_test/CMakeLists.txt"
    "tools/hardware/gpio_test/build.sh"
    "tools/hardware/gpio_test/main.c"
    "tools/hardware/relay_test/CMakeLists.txt"
    "tools/hardware/relay_test/build.sh"
    "tools/hardware/relay_test/main.c"
    "vision_analysis/src/gpio/gpio.cpp"
    "vision_analysis/include/rkvision/gpio.h"
...
# setup/install_deps.sh 会在严格核验后把兼容元数据写入 /usr/local。若该文件不归
