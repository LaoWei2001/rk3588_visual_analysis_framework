...
安装器模板，不会调用 `setup/offline/debian/`。制包仍需父目录中的项目源码，
因此应复制整个项目到制作机；可以删除其中的 `setup/offline/debian/`，不会影响
...
# 一条命令自动准备制作机并生成完整开发包
bash setup/offline/ubuntu/create_bundle.sh
```
...
```text
setup/offline/ubuntu/output/full-bundle
```

生成后核对元数据：

```bash
sed -n '1,30p' setup/offline/ubuntu/output/full-bundle/BUNDLE_INFO
```
