#!/usr/bin/env bash
# 在联网 Ubuntu ARM64 RK3588 上生成可直接安装的独立离线仓库。
set -Eeuo pipefail
...
ENV_DISPLAY_PATH="setup/offline/ubuntu"
STRICT_TARGET_OS=true
...
无需参数。脚本会自动准备联网 Ubuntu 制作机、刷新依赖、临时编译并识别
...
[ -f "$PREPARE_HOST" ] \
    || { echo "[错误] 缺少 Ubuntu 制作机准备脚本: $PREPARE_HOST" >&2; exit 1; }
echo ">>> [准备] 自动安装并检查 Ubuntu 制作机依赖..."
...
# Ubuntu 包必须来自制作机当前启用的 Ubuntu 与 Rockchip BSP 软件源，确保
# 厂商 GStreamer、RGA、MPP 包与目标镜像保持同一套版本。
SOURCE_APT_OPTIONS=()
echo "    复用制作机准备阶段刚刚更新的 Ubuntu APT 索引。"
