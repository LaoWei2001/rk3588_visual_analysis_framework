...
| 文件 | 职责 |
|------|------|
| `analyzer.h / analyzer.cpp` | 分析器生命周期(`analyzer_init/deinit`)、显示队列管理、ROI 加载、通道在线/离线通知 |
| `analyzer_internal.h` | 各 .cpp 共享的内部状态（显示队列、推理标志等），不对外暴露 |
| `frame_inlet.cpp` | **`videoOutHandle` 帧处理总入口**、FPS 节流决策（是否送推理）、首帧黑图兜底、帧间隔 `dt_ms` 计算、RGA 转换调用 |
...
GStreamer appsink 回调（解码线程）
    │
    ▼
videoOutHandle()               ← 帧处理总入口（frame_inlet.cpp）
    │
...
// out_frame_id：该帧序号（与 out_frame 同帧，用于帧-结果配对校验）
...
struct ChannelContext {
    ...
    uint64_t        unix_ms;          // 墙上时钟 Unix epoch 毫秒（真实时间，RTSP/USB/文件 三源统一）
    float           dt_ms;            // 与上次 logic 调用的时间差（首次=0）
    vector<AlgoResult> *results;      // 当前帧检测结果（可修改 box_color）
    const ChannelConfig *config;      // 通道配置（只读）
    const vector<cv::Point> *roi;     // 兼容字段：第 0 个 ROI 多边形（模型坐标系）；无 ROI 时为空
    const vector<RoiZone>   *rois;    // 本通道全部 ROI 区域（多区域，各含名字+多边形）
    ...
    // 注意：没有 fn 字段；逻辑函数指针在分发表(LogicEntry)里，不在 ctx 上。

    // 便捷方法（详见 channel_logic.h / channelcontext-api.md）
    int    has_target(const char *label) const;      // 整帧
    int    target_count(const char *label) const;    // 整帧
    // ROI 查询是【自由函数】: roi_contains / roi_has_target / roi_count_target(ctx, …, idx)
    std::string time_hms() const;     // "HH:MM:SS"（unix_ms 格式化）
    FrameTime   datetime() const;     // 拆成年月日时分秒(各 int)
...
| `logic_server` | 有检测结果时上报到业务服务器（落本地发件箱→Python 补传） |
...
struct GlobalContext {
    ...
    uint64_t timestamp_ms;               // 本 tick 时间戳（单调，限频用）
    int64_t  tick_id;                    // 累计 tick 计数
    const vector<int> *channel_ids;      // 监控的通道号列表（用 for_each_channel 遍历）
    int      has_new_infer;              // 本 tick 是否有受监控通道出新结果
    int      latest_infer_channel;       // 最近出结果的通道（无则 -1）
    uint64_t latest_infer_ts_ms;         // 该通道出结果的时刻
    shared_ptr<void> *state;             // 本实例跨 tick 状态（自行 cast）

    // 便捷方法（完整以 global_logic.h 为准）
    ChannelSnapshot get_channel_snapshot(int chnId) const;        // 原子取 帧+结果+logic_state（同帧，按值返回）
    string get_channel_logic_name(int chnId) const;
    int    channel_has_logic(int chnId, const char *name) const;
    ...
    template<typename Fn> void for_each_channel(Fn &&fn) const;   // 回调 (int chnId, int idx)
};
// 注意：没有 all_results / get_channel_results / get_channel_frame / get_channel_state(_fresh)。
// 跨通道读“帧/结果/状态”一律走 get_channel_snapshot(ch)，它一次性原子取出三者。
```
...
- 逻辑里访问：`ctx->rois`（全部区域）、`ctx->roi_by_name("entrance")`、`ctx->roi_polygon_at(i)`、
  `roi_count_target(ctx, "person", i)` 等；`ctx->roi` 仍指向第一个区域（兼容老逻辑）。
...
static void global_my_logic(GlobalContext *gctx) {
    ...
    if (alarm_channels >= 2) {
        ...
        gctx->for_each_channel([&](int chnId, int) {
            ...
            if (!snap.frame.empty() && snap.has_results) {
...
**步骤二：** 在 `global_logic.cpp` 里用注册接口登记（见文件内 "新增 global logic" 注释处）：
...
```cpp
// HookState 定义在 logic_tools.h；从快照的 logic_state 原子取（与 frame/results 同帧）
ChannelSnapshot s = gctx->get_channel_snapshot(0);
if (!s.frame.empty() && s.result_age_ms < 500 && s.logic_state) {
    auto hook = std::static_pointer_cast<HookState>(s.logic_state);
    if (hook->alarm_sent_latch) {
        // 通道 0 的 hook logic 在 500ms 内发送过报警
    }
}
```

跨通道读状态**没有**单独的 `get_channel_state` 方法——一律走 `get_channel_snapshot(ch)`：它在一把锁内把 `frame + results + logic_state` 原子取出（保证同帧），再 `static_pointer_cast` 成你的状态类型；用 `s.result_age_ms` 自检新鲜度。
