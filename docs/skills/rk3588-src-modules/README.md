...
| `src/event/` | 标准事件建档、图片生成、投递清单和事件合并 | [event.md](event.md) |
...
| 外部上传服务 | 消费 `event_store` 发件箱并发送到服务器/Dify；不是当前 `src/` 模块 | [uploader.md](uploader.md) |
                              ...
                              -> 原子写回同帧 frame/results/draw_cmds/state
                                      -> report_event
                                         -> event_store/<event_id>/
                                            event.json + media_state.json + delivery_state.json
                                            + raw.jpg / annotated.jpg
...
| 提交事件并生成图片/视频 | [event.md](event.md)、[recorder.md](recorder.md) |
