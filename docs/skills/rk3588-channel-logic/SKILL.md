...
> **名字必须对齐**：`REGISTER_LOGIC` 的**字符串** == `logics.json` 的 `name` == `config.json` 的 `logic`（文件名 / C++ 函数名只是约定，对外不可见，是哪个无所谓）。这套"四名关系 + 网页怎么认出逻辑 + 名字失配会怎样"的完整说明见 `references/logic-naming-and-registration.md`。注意网页「逻辑名称」现在**只能从 `logics.json` 下拉选、不能手填**，所以一个逻辑要能在网页被选到，必须在 `logics.json` 声明（光 `REGISTER_LOGIC` 只保证能跑、不保证网页选得到）。
