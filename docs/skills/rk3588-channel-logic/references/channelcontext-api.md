...
| `ctx->frame` | `const cv::Mat *` | 当前帧（BGR，**模型输入尺寸**，通常 640×640）。**可能为空，先判空** |
| `ctx->src_width` / `ctx->src_height` | `int` | **原始视频分辨率**（摄像头/视频源真实尺寸，如 1920×1080）；区别于 `ctx->frame` 的模型输入尺寸。首帧解码前可能为 0 |
...
int  roi_contains(const ChannelContext *ctx, const cv::Rect &box, int idx);   // 框中心是否在 ROI 内（无 ROI 返回 1）
...
| `ctx->roi_index_of(box)` | 框中心落在第几个区域（取首个命中；都不在/无区域 → `ROI_NONE`，可直接回传给 `roi_*` 自由函数，不会被误当 `ROI_ALL`） |
| `ChannelContext::point_box_in_poly(poly, box)`（静态） | 框中心是否在给定多边形内（多边形 <3 点视为"全屏"=1） |

> 单区域、多区域用同一套 C 风格自由函数 `roi_contains` / `roi_has_target` / `roi_count_target`(`idx` 传 `ROI_ALL` = 所有区域)。多区域完整范例见 `examples/logic_multi_roi.md`、`examples/logic_wafer_sop.md`（按名字把业务步骤绑到各区域）。
...
void draw_polyline(ctx, const std::vector<cv::Point> &pts, color=绿, thickness=2, alpha=1.0, closed=false, target=ALL);
void draw_poly_filled(ctx, const std::vector<cv::Point> &pts, color=绿, alpha=0.3, target=ALL);  // 填充多边形(色块)
```
颜色是 BGR：红 `cv::Scalar(0,0,255)`、绿 `cv::Scalar(0,255,0)`、黄 `cv::Scalar(0,255,255)`。

**填充 / 半透明 / 加粗**：
- `thickness = -1`（负数）→ 填充矩形/圆（实心）。
- `alpha < 1` → 半透明叠加（矩形/圆/折线/填充多边形都支持；目标/画面能透出来，适合高亮报警区）。
  例：`draw_rect(ctx, zone, 红, -1, 0.3)` 半透明红块；`draw_poly_filled(ctx, *ctx->roi, 红, 0.3)` 给不规则 ROI 铺半透明红底。
- `draw_text` 的 `thickness` = **加粗级别**：`<=1` 普通字（默认），`>=2` 越大越粗（报警大字想醒目就调大，如 `draw_text(ctx,"报警",{180,40},红,1.2,4)`）。
...
文件**末尾**写一行 `REGISTER_LOGIC("logic_xxx", logic_xxx);` 即自注册到分发表（在 `main()` 之前生效），**无需改动 `channel_logic.cpp`**；删除该逻辑＝删除该文件。`src/logic` 下的 `.cpp` 由 CMake 自动收集编译。
