...
现有服务器固定协议始终同时发送 `base64Data` 和 `base64DataRaw`：前者是上述选项决定的 `annotated.jpg`，后者始终是无叠加的 `raw.jpg`。因此选择“与实时播放窗口画面一致”时，服务器一次请求即可同时取得带信息截图和原始帧；选择“当前原始帧”时两个字段内容都不带叠加。Dify 图片投递只发送 `annotated.jpg`。
