...
新建项目：从仓库根目录执行 `./vision create projects/my_app`，随后进入新项目运行 `./build.sh`。
也可以把项目放在仓库外，使用 `./build.sh --engine /path/to/framework`。
...
`build.sh` 自动复用框架 `build/runtime/` 的引擎缓存；发布包始终包含 `libs/librkvision.so.1`。
