...
## 实现位置
代码在 `channel_logic.cpp` 的 `logic_wafer2` 及 `w2_*` 辅助函数（`w2_reversal_rate` 折返率、`w2_pca_angle_smoothed` PCA 角、`w2_segment_trajectory` 分段、`w2_extract_ellipse` 椭圆提取），跨帧状态 `Wafer2State`（轨迹/椭圆/覆盖掩码/分段缓存）。

## 接线
- 注册：`register_logic("logic_wafer2", logic_wafer2);`
