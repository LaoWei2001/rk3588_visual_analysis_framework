...
```
`wafer_window_label` / `wafer_motion_features`(PCA `linearity` + 逐步行程 + Kåsa 圆拟合 + Hann+`cv::dft` 主频占比) 全实现见 `channel_logic.cpp`。
> 阈值（窗口28、支持度 2 或 12%、残差0.12、角程1.0、行程比1.15）按现场视频微调。

## 跨帧状态（要点）
```cpp
struct WaferState {
    std::vector<cv::Point> trajectory;   // 轨迹点
    std::vector<int>       traj_action;  // 平行：每点所属动作(多色绘制/完整性)
    cv::Mat swept_mask; cv::Size mask_size;
    ...
    int cur_action=0; bool seen_h=false, seen_v=false, seen_arc=false;  // 当前/已出现动作
    bool have_result=false, last_pass=true; float last_coverage=0, last_op_sec=0; std::string last_msg; // 结算
};
```
完整实现见 `rk3588_yolo/src/logic/channel_logic.cpp` 的 `logic_wafer`（含 `wafer_action_color/name`、`wafer_classify_action`、`wafer_missing_actions` 四个 helper）。关键流程：找 wafer 框→guard→扫描 cleanwiper(更新 region_last_ms)→开始确认→记点+识别动作→重建掩码算实时覆盖→结束判定+结算→多色绘制+左上文字+右上报警。

## 接线
- 注册：`register_logic("logic_wafer", logic_wafer);`
