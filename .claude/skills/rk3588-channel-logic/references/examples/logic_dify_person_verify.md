...
```
> 完整逐框标注绘制见源码 `channel_logic.cpp` 的同名函数（约第 757–815 行）。跨帧状态 `DifyPersonVerifyState`（含 `reported_ids` set、`miss_frames` map、`last_upload_ms`、`first`）在 `logic_tools.h`。

## 接线
- 注册：`register_logic("logic_dify_person_verify", logic_dify_person_verify);`
