...
`rk3588_yolo/src/logic/channel_logic.cpp` 的 `channel_logic_init()` 里，找到注释
`/* 新增 logic: 在此处添加 register_logic("logic_xxx", logic_xxx); 即可 */`，在它上面加：
```cpp
...
```
- `name`：必须与 `register_logic` 的名字、config.json 里 `"logic"` 的值完全一致。
- `label`：网页下拉里显示的中文名。
