...
1. **`rk3588_yolo/src/logic/channel_logic.cpp`** — 新增 `static void logic_xxx(ChannelContext *ctx)` 函数；并在 `channel_logic_init()` 里（约第 901 行的 `/* 新增 logic: ... */` 标记处）加一行 `register_logic("logic_xxx", logic_xxx);`从而注册该逻辑。
...
1. **注册**（channel_logic.cpp，`channel_logic_init()` 内的标记处）：
...
1. **`channel_logic.cpp`** —— 同删两处（互相引用，必须一起删）：`channel_logic_init()` 里的 `register_logic("logic_foo", logic_foo);`，以及函数定义 `static void logic_foo(ChannelContext *ctx){ ... }`；连同**仅它使用**的辅助结构/函数（如 `struct FooState`、专用 helper），避免死代码 / 未使用告警。
