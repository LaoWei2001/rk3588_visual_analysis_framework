...
#define GPIO_LINE_OFFSET 0U /* GPIO6_A0，沿用 08_GPIO/test-gpio */
#define GPIO_CONSUMER "gpio_test"
#define STATE_FILE "/var/lib/gpio6_a0_test.state"
...
int main(void)
{
    ...
    if (!line)
    {
        fprintf(stderr, "无法获取 GPIO6_A0：%s\n", strerror(errno));
        goto cleanup;
    }
    ...
    {
        fprintf(stderr, "无法读取 GPIO6_A0 当前状态：%s\n", strerror(errno));
        goto cleanup;
    }
    ...
    {
        fprintf(stderr, "无法控制 GPIO6_A0：%s\n", strerror(errno));
        goto cleanup;
    }
    readback = gpiod_line_get_value(line);
    if (readback != next_value)
    {
        fprintf(stderr, "GPIO6_A0 输出校验失败，期望 %d，读取到 %d\n",
                next_value, readback);
        goto cleanup;
    }
    if (save_value(state_fd, next_value) != 0)
    {
        fprintf(stderr, "GPIO 已切换，但无法保存状态：%s\n", strerror(errno));
        goto cleanup;
    }
