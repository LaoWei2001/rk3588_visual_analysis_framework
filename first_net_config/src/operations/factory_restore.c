...
void capture_factory_baseline(void)
{
    ...
    printf("\n此操作将把当前配置定为系统初始状态。\n");
    printf("仅限制作镜像或设备未被现场修改时执行。\n");
    terminal_ui_set_step("确认保存初始基准",
                         "仅限制作镜像或确认设备仍为初始状态时执行");
...
void preview_factory_restore(void)
{
    ...
    show_network_state();
    printf("\n========== 恢复内容 ==========\n");
    printf("• 初始的有线和 Wi-Fi 保存连接\n");
    printf("• 初始的摄像头应用设置\n");
    printf("恢复后重启生效，当前连接不会立即断开。\n");
}
...
void restore_factory_network(void)
{
    ...
    terminal_ui_set_step("确认恢复初始网络",
                         "恢复在重启后生效，并可能改变 SSH 地址");
    terminal_ui_prepare_step(false);
    ...
    written = snprintf(emergency, sizeof(emergency),
                       "%s/before-factory-reset-%s.tar", backups, stamp);
    if (written < 0 || (size_t)written >= sizeof(emergency))
    {
        return;
    }
    written = snprintf(emergency_camera, sizeof(emergency_camera),
                       "%s/before-factory-reset-%s.camera.json",
                       backups, stamp);
    if (written < 0 || (size_t)written >= sizeof(emergency_camera))
    {
        return;
    }
    ...
    printf("\n[完成] 已恢复初始网络与摄像头配置，未主动断开当前连接。\n");
    printf("请在本机操作条件下重启后生效。恢复前备份：%s\n", emergency);
}
