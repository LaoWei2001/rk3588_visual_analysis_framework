...
static void rotate_y(double angle, double x, double y, double z, double *out_x,
...
static void rotate_x(double angle, double x, double y, double z, double *out_x,
                     double *out_y, double *out_z) {
  double cosine = cos(angle);
  double sine = sin(angle);
...
static void rotate_z(double angle, double x, double y, double z, double *out_x,
                     double *out_y, double *out_z) {
  double cosine = cos(angle);
  double sine = sin(angle);
...
static void rotate_cube_vector(const CubeFrame *frame, double x, double y,
                               double z, double *out_x, double *out_y,
                               double *out_z) {
  double first_x;
  double first_y;
  double first_z;
  ...
  rotate_y(frame->angle, x, y, z, &first_x, &first_y, &first_z);
  rotate_x(frame->pitch, first_x, first_y, first_z, &second_x, &second_y,
           &second_z);
  rotate_z(frame->roll, second_x, second_y, second_z, out_x, out_y, out_z);
}

static void cube_plot(CubeFrame *frame, double x, double y, double z, double nx,
                      double ny, double nz, bool edge, char greeting) {
  ...
  rotate_cube_vector(frame, x, y, z, &rotated_x, &rotated_y, &rotated_z);
  rotate_cube_vector(frame, nx, ny, nz, &rotated_nx, &rotated_ny, &rotated_nz);
  ...
  screen_x =
      frame->columns / 2 + (int)lround(rotated_x * frame->scale * perspective);
  screen_y = frame->rows / 2 -
             (int)lround(rotated_y * frame->scale * 0.50 * perspective);
  if (screen_x < 1 || screen_x >= frame->columns - 1 || screen_y < 0 ||
      screen_y >= frame->rows) {
    return;
  }
  ...
  diffuse = rotated_nx * light_x + rotated_ny * light_y + rotated_nz * light_z;
  if (diffuse < 0.0) {
    diffuse = 0.0;
  }
  brightness = 0.18 + diffuse * 0.82;
  tone = 1 + (int)lround(brightness * (double)(sizeof(light_ramp) - 3));
  if (tone < 1) {
    tone = 1;
  }
  ...
  frame->pixels[index] = greeting != '\0' ? greeting
                         : edge           ? '@'
                                          : light_ramp[tone];
...
static char hello_face_character(double horizontal, double vertical) {
  ...
  if (horizontal < left || horizontal >= right || vertical <= bottom ||
      vertical > top) {
    return '\0';
  }
  column = (int)((horizontal - left) * HELLO_TEXT_WIDTH / (right - left));
  row = (int)((top - vertical) * HELLO_GLYPH_HEIGHT / (top - bottom));
  if (column < 0 || column >= HELLO_TEXT_WIDTH || row < 0 ||
      row >= HELLO_GLYPH_HEIGHT) {
    return '\0';
  }
...
static void cube_face(CubeFrame *frame, int axis, int side) {
  for (int row = 0; row <= CUBE_FACE_STEPS; ++row) {
    ...
    for (int column = 0; column <= CUBE_FACE_STEPS; ++column) {
      ...
      bool edge = row == 0 || row == CUBE_FACE_STEPS || column == 0 ||
                  column == CUBE_FACE_STEPS;
      char greeting = '\0';
      ...
      cube_plot(frame, point[0], point[1], point[2], normal[0], normal[1],
                normal[2], edge, greeting);
    }
  }
}
...
void cube_intro_play(void) {
  ...
  flushinp();
  nodelay(stdscr, true);
  for (int animation_frame = 0; animation_frame < CUBE_FRAMES;
       ++animation_frame) {
    int key;
    ...
    frame.angle = 2.0 * CUBE_PI * animation_frame / (CUBE_FRAMES - 1);
