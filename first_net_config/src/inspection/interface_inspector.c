...
static int route_metric_for_network(const char *iface,
                                    uint32_t network,
                                    int prefix)
{
    FILE *handle = fopen("/proc/net/route", "r");
    char line[512];
    uint32_t expected_mask = prefix == 32
        ? UINT32_MAX
    ...
    while (fgets(line, sizeof(line), handle))
    {
        ...
        if (sscanf(line, "%15s %lx %lx %lx %lu %lu %lu %lx",
                   route_iface, &destination, &gateway, &flags,
                   &ref_count, &use_count, &metric, &mask) != 8 ||
            strcmp(route_iface, iface) != 0 ||
            (flags & 0x1UL) == 0 ||
            ntohl((uint32_t)destination) != network ||
            ntohl((uint32_t)mask) != expected_mask ||
            metric > INT_MAX)
        {
            continue;
        }
        ...
        (void)ref_count;
        (void)use_count;
        if (best_metric < 0 || (int)metric < best_metric)
        {
            best_metric = (int)metric;
        }
    }
    fclose(handle);
    return best_metric;
...
static int collect_ipv4_rows(IPv4Row *rows, int capacity)
{
         ...
         item && count < capacity; item = item->ifa_next)
    {
        ...
        rows[count].route_metric = route_metric_for_network(
            item->ifa_name, rows[count].first, prefix);
        ++count;
    }
...
static bool same_network(const IPv4Row *left, const IPv4Row *right)
{
...
static bool priority_controls_overlap(const IPv4Row *left,
                                      const IPv4Row *right)
{
    return strcmp(left->ip, right->ip) != 0 &&
           same_network(left, right) &&
           left->route_metric >= 0 &&
           right->route_metric >= 0 &&
           left->route_metric != right->route_metric;
}

static void show_controlled_overlap(const IPv4Row *left,
                                    const IPv4Row *right,
                                    const char *network)
{
    const IPv4Row *primary = left->route_metric < right->route_metric
        ? left
        : right;
    const IPv4Row *backup = primary == left ? right : left;

    printf("[主备网络] %s 与 %s 同属 %s，但路由优先级明确不同。\n",
           left->iface, right->iface, network);
    printf("           主用：%s %s 优先级 %d；备用：%s %s 优先级 %d。\n",
           primary->iface, primary->ip, primary->route_metric,
           backup->iface, backup->ip, backup->route_metric);
}
...
static bool sys_path_exists(const char *iface, const char *leaf)
{
    ...
    if (!iface || !leaf ||
        (written = snprintf(path, sizeof(path),
                            "/sys/class/net/%s/%s", iface, leaf)) < 0 ||
                                       ...
                                       const char *expected)
{
    ...
    if (!iface || !expected ||
        (written = snprintf(path, sizeof(path),
                            "/sys/class/net/%s/uevent", iface)) < 0 ||
        (size_t)written >= sizeof(path))
    {
        return false;
    }
...
void show_interface_overview(void)
{
         ...
         entry->if_index != 0 && entry->if_name; ++entry)
    {
        ...
        printf("%-10s 类型:%-6s 连接:%-8s MAC:%s%s%s\n",
               ...
               carrier, mac,
               interface_has_default_route(entry->if_name) ? " [默认联网]" : "",
...
int show_current_overlap_warnings(void)
{
    ...
    for (int left = 0; left < count; ++left)
    {
        for (int right = left + 1; right < count; ++right)
        {
            ...
            if (strcmp(rows[left].ip, rows[right].ip) == 0)
            {
                printf("[严重警告] 重复 IP：%s 和 %s 都在使用 %s。\n",
                       rows[left].iface, rows[right].iface, rows[left].ip);
                ++warnings;
                continue;
            }
            if (priority_controls_overlap(&rows[left], &rows[right]))
            {
                show_controlled_overlap(&rows[left], &rows[right],
                                        left_network);
                ++controlled;
                continue;
            }
    ...
    if (warnings == 0 && controlled == 0)
    {
        printf("[正常] 当前各网卡 IPv4 网段没有重叠。\n");
    }
    else if (warnings == 0)
    {
        printf("[正常] 重叠网段具有明确且不同的路由优先级，按主备网络处理。\n");
    }
    return warnings;
}
                               ...
                               int route_metric)
{
    ...
    candidate.route_metric = route_metric;
    count = collect_ipv4_rows(rows, MAX_IPV4_ROWS);
    ...
    for (int index = 0; index < count; ++index)
    {
        ...
        format_network(&rows[index], network, sizeof(network));
        if (priority_controls_overlap(&candidate, &rows[index]))
        {
            show_controlled_overlap(&candidate, &rows[index], network);
            controlled = true;
            continue;
        }
    ...
    if (!overlap)
    {
        if (controlled)
        {
            printf("[允许] 不同 IP、同网段、不同优先级，按主备处理；优先级只定首选网卡，不叠加带宽。\n");
        }
        return true;
    }
    printf("相同或重叠网段出现在不同网卡上，数据可能走错网卡。\n");
    terminal_ui_set_step("确认网段冲突",
                         "存在重叠网段，只有确认风险后才能继续");
    return read_exact_word("确认继续请输入 OVERLAP；否则取消: ",
                           "OVERLAP");
}
