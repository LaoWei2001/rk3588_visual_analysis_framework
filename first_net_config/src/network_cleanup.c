...

    {
        ...
        while (line)
        {
            ...
            if (length < UUID_SIZE || line[UUID_SIZE - 1] != ':' ||
                count >= capacity)
            {
                printf("[拒绝] NetworkManager 连接清单格式异常或数量过多。\n");
                return -1;
            }
            ...
            {
                printf("[拒绝] NetworkManager 返回了无效 UUID。\n");
                return -1;
            }
...
static void clear_persistent_after_reboot(void)
{
    ...
    printf("\n[永久操作] 将删除磁盘上的全部 NetworkManager 永久连接。\n");
    printf("程序不会主动 reload、重启 NetworkManager 或断开当前连接；\n");
    printf("但 NetworkManager 可能自行检测文件变化，SSH 仍有中断风险。\n");
    ...
    printf("\n[完成] 永久连接文件已清空，程序没有主动断开当前网络。\n");
    printf("重启后，当前临时/内存连接也会消失。紧急备份：%s\n", backup);
}
...
static void clear_immediately(ClearConnection *connections, int count)
{
    ...
    printf("\n[极高风险永久操作] 将立即删除临时、永久和活动连接。\n");
    printf("所有有线/Wi-Fi 网络都可能中断；请按当前 SSH 一定会断开准备。\n");
    printf("程序会先删除未启用连接，再删除其他活动连接，最后删除当前 SSH 连接。\n");
    ...
    if (failures == 0)
    {
        printf("[完成] NetworkManager 当前连接 profile 及残留 keyfile 已清空。\n");
    }
    else
    {
...
void clear_all_network_connections(void)
{
    ...
    printf("\n========== 清除所有 NetworkManager 连接配置 ==========\n");
    printf("本功能不依赖初始网络基准，也不会删除系统驱动、NetworkManager、\n");
    printf("系统级策略、初始基准或 Web Console 摄像头发现信息。\n");
    printf("\n--- 当前 NetworkManager 连接清单 ---\n");
    (void)run_cmd(list);
    ...
    printf("\n共检测到 %d 个当前已知连接 profile。\n", count);
    printf("请选择清除方式：\n");
    printf("  1. 只清除永久连接文件，重启后全部失效（较安全）\n");
    printf("  2. 立即清除全部连接，包括活动连接（SSH 会断开）\n");
    printf("  0. 取消\n");
